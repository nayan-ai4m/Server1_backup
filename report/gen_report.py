...
def fetch_status_groups(
    host, dbname, user, password, table="mc17", port=5432, timezone="Asia/Kolkata"
):

    # Get current time and time 8 hours 2 minutes ago in proper timezone
    tz = pytz.timezone(timezone)
    current_time = datetime.now(tz)
    start_time = current_time - timedelta(hours=8, minutes=2)
...
def get_json(json_file_path):
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            raise ValueError(
                "Unexpected JSON format: please provide a list of dictionaries or specify the correct key."
            )
...
def save_state_diagram(df, filename="state_diagram.png"):
    ...
    try:
        ...
        df["color"] = df.apply(
            lambda row: (
                "green" if row["status"] == 1.0 and row["status_code"] == 0.0 else "red"
            ),
            axis=1,
        )
...
def get_state_dataframe(df, codes):
    ...
    try:
        ...
        state_df.loc[
            (state_df["status"] == 1) & (state_df["status_code"] == 0), "message"
        ] = "Running"

        # Calculate duration in HH:MM:SS (total hours even beyond 24)
        state_df["duration"] = (
            (state_df["time_end"] - state_df["time_start"])
            .dt.total_seconds()
            .apply(
...
def calc_run(state_df):
    ...
    try:
        ...
        running_durations = state_df[
            (state_df["status"] == 1) & (state_df["status_code"] == 0)
        ]["duration"]
...
def get_reasionwise_stoppage(state_df):
    ...
    try:
        ...
        aggregated_df["total_stoppage_duration"] = aggregated_df["duration"].apply(
            lambda x: str(x).split()[2]
        )

        aggregated_df = aggregated_df[["message", "total_stoppage_duration"]]

        aggregated_df["frequency"] = aggregated_df["message"].apply(
            lambda x: stoppage_df[stoppage_df["message"] == x].shape[0]
        )
...
def get_top_stoppage_reasons(df):
    ...
    try:
        if (
            df.empty
            or "message" not in df.columns
            or "total_stoppage_duration" not in df.columns
            or "frequency" not in df.columns
        ):
            raise ValueError(
                "DataFrame must contain 'message', 'total_stoppage_duration', and 'frequency' columns."
            )
...
def create_shift_analysis_df(running_sum, stoppage_sum, duration_message, freq_message):
    ...
    try:
        ...
        data = {
            ...
            "Output": [running_sum, stoppage_sum, duration_message, freq_message],
        }
...
def main():
    ...
    try:
        ...
        freq_message, freq_value, duration_message, duration_value = (
            get_top_stoppage_reasons(reasonwise_stoppage)
        )

        shift_analysis = create_shift_analysis_df(
            running_sum, stoppage_sum, duration_message, freq_message
        )
