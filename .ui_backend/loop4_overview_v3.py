...
def load_override_config():
    ...
    try:
        override_file = "loop4_config_overrides.json"
        if os.path.exists(override_file):
            with open(override_file) as f:
                override_config = json.load(f)
                if override_config.get("override_enabled", False):
                    logging.warning("⚠️  OVERRIDE MODE ENABLED - Using hardcoded values from loop4_config_overrides.json")
                return override_config
        else:
            logging.info("No override config found, using normal mode")
            return {"override_enabled": False, "overrides": {}}
    except Exception as e:
        logging.error(f"Error loading override config: {e}, using normal mode")
        return {"override_enabled": False, "overrides": {}}
...
class Loop4Monitor:
    def __init__(self, db,write_db,config):
        ...
        self.override_config = load_override_config()
        self.override_enabled = self.override_config.get("override_enabled", False)
        ...
        self.GET_PREVIOUS_SHIFT_MACHINES_QUERY = """
            SELECT machines
            FROM loop4_overview_new
            WHERE shift = %s
            ORDER BY last_update DESC
            LIMIT 1
        """

    def apply_override(self, key, calculated_value, override_dict=None, machine_num=None):
        """
        Apply override value if override mode is enabled and value exists.

        Args:
            key: The field name to check for override
            calculated_value: The real calculated/fetched value
            override_dict: Optional specific override dictionary to use
            machine_num: Optional machine number (e.g., 'mc25') for machine-specific overrides

        Returns:
            Override value if exists and enabled, otherwise calculated_value
        """
        ...
        try:
            if override_dict is None:
                # Use common_metrics or machines based on machine_num
                if machine_num:
                    override_dict = self.override_config.get("overrides", {}).get("machines", {}).get(machine_num, {})
                else:
                    override_dict = self.override_config.get("overrides", {}).get("common_metrics", {})

            # Check if override exists and is not None
            if override_dict and key in override_dict and override_dict[key] is not None:
                override_value = override_dict[key]
                logging.info(f"🔧 Override applied for '{key}' on {machine_num or 'common'}: {override_value}")
                return override_value

            return calculated_value
        except Exception as e:
            logging.error(f"Error applying override for {key}: {e}")
            return calculated_value

    def calculate_expected_cld(self, machine_number, elapsed_seconds):
        """Calculate expected CLD production based on elapsed time in current shift"""
        # Production rates: seconds per CLD
        # 8 hours = 28800 seconds
        production_rates = {
            25: 28800 / 720,  # MC25: 40 seconds per CLD
            26: 28800 / 720,  # MC26: 40 seconds per CLD
            30: 28800 / 630,  # MC30: 45.714... seconds per CLD
            # MC27, MC28, MC29: 600 CLDs in 8 hours = 48 seconds per CLD (default)
        }
        production_rate = production_rates.get(machine_number, 28800 / 600)  # Default: 48 seconds per CLD
        return round(elapsed_seconds / production_rate)

    def get_fixed_expected_cld(self, machine_number):
        """Get fixed expected CLD for complete 8-hour shift (previous shift)"""
        if machine_number in [25, 26]:
            return 720
        elif machine_number == 30:
            return 630
        else:  # MC27, MC28, MC29
            return 600

    def get_previous_shift(self, current_shift):
        """Get the previous shift based on current shift"""
        shift_sequence = {"A": "C", "B": "A", "C": "B"}
        return shift_sequence.get(current_shift, "A")

    def get_previous_shift_cld(self, machine_key, previous_shift):
        """Query previous shift's CLD production for a specific machine"""
        try:
            self.db.cursor.execute(self.GET_PREVIOUS_SHIFT_MACHINES_QUERY, (previous_shift,))
            result = self.db.cursor.fetchone()

            if result and result[0]:
                machines_data = result[0]
                if isinstance(machines_data, dict) and machine_key in machines_data:
                    return machines_data[machine_key].get('cld_production', 0)
            return 0
        except Exception as e:
            logging.error(f"Error fetching previous shift CLD for {machine_key}: {e}")
            return 0
    ...
    def get_machine_25(self,start_time,end_time,shift):
        print("\n---------------------------------------------------")
        print("\n\n machine number : mc25")
        print("\n---------------------------------------------------")

        # Fetch laminate waste data
        self.cursor.execute(self.LAMINATE_LENGTH_QUERY, ('mc25', start_time, end_time))
        laminate_result = self.cursor.fetchall()
        if laminate_result:
            total_waste_amount_cm, empty_duration = laminate_result[0]
            laminate_length = total_waste_amount_cm / 100  # Convert cm to meters
        else:
            laminate_length = 0
        ...
        time_difference_hours = elapsed_seconds / 3600

        # Calculate expected CLD for current shift based on elapsed time
        expected_cld = self.calculate_expected_cld(25, elapsed_seconds)
        print(f"Expected CLD (current shift): {expected_cld}")

        # Calculate current BBOEE using new formula
        BBOEE = (cld_production / expected_cld) * 100 if expected_cld > 0 else 0
        BBOEE = max(0, min(BBOEE, 100))

        # Get previous shift expected CLD (fixed for 8 hours)
        prev_expected_cld = self.get_fixed_expected_cld(25)

        # Get previous shift CLD production
        prev_shift = self.get_previous_shift(shift)
        prev_cld_production = self.get_previous_shift_cld("mc25", prev_shift)
        print(f"Previous shift CLD production: {prev_cld_production}")
        print(f"Previous expected CLD: {prev_expected_cld}")

        # Calculate previous BBOEE
        prev_bboee = (prev_cld_production / prev_expected_cld) * 100 if prev_expected_cld > 0 else 0
        prev_bboee = max(0, min(prev_bboee, 100))
        print(f"Previous BBOEE: {prev_bboee}")
        ...
        try:
            ...
            if avg_data_result:
                ...
                mc_data = {'speed': 120 ,'status':status_message,'laminate_length': round(laminate_length, 2),'laminate_duration': str(empty_duration),'jamming':True,'pulling':True,'sealing':True,'sealing_time_vertical':192,'sealing_time_horizontal':195,'stoppage_time':round(stoppage_time_minute),"longest_duration" : longest_duration,'cld_production':cld_production,'stoppage_count':int(stoppage_count) if stoppage_count is not None else 0,'outgoing_quality':True,'vertical_pressure':round(avg_data[2], 2) if avg_data[2] is not None else 0,'vertical_rear_temp':round(avg_data[6],2) if avg_data[6] is not None else 0,
                   'horizontal_pressure':round(avg_data[3], 2) if avg_data[3] is not None else 0,'vertical_front_temp':round(avg_data[7],2) if avg_data[7] is not None else 0,'breakdown_prediction': True,'hopper_level_primary': round(hopper_data[0], 2) if hopper_data[0] is not None else 0,'horizontal_rear_temp':round(avg_data[8],2) if avg_data[8] is not None else 0,'horizontal_front_temp':round(avg_data[9],2) if avg_data[9] is not None else 0,'hopper_level_secondary': round(hopper_data[1], 2) if hopper_data[1] is not None else 0,'productivity_prediction': True,'machine_uptime' : str(machine_uptime),'bboee':round(BBOEE,2),'expected_cld_production':expected_cld,'prev_expected_cld_production':prev_expected_cld,'prev_cld_production':prev_cld_production,'prev_bboee':round(prev_bboee,2)
                ...
                'speed': 0,'status': "Offline",'laminate_length': round(laminate_length, 2),'laminate_duration': str(empty_duration),'jamming': True,'pulling': True,'sealing': True,'sealing_time_vertical':0,'sealing_time_horizontal': 0,'stoppage_time':round(stoppage_time_minute) ,"longest_duration" : longest_duration,'cld_production':cld_production,'stoppage_count': int(stoppage_count) if stoppage_count is not None else 0,'outgoing_quality': True,'vertical_pressure': 0,'vertical_rear_temp': 0,
                    'horizontal_pressure': 0,'vertical_front_temp':0,'breakdown_prediction': True,'hopper_level_primary': 0,'horizontal_rear_temp':0,'horizontal_front_temp': 0,'hopper_level_secondary': 0,'productivity_prediction': True,'machine_uptime' : str(machine_uptime),'bboee':round(BBOEE,2),'expected_cld_production':expected_cld,'prev_expected_cld_production':prev_expected_cld,'prev_cld_production':prev_cld_production,'prev_bboee':round(prev_bboee,2),"prev_stoppage_time" : round(prev_stoppage_minutes),"prev_stoppage_count": int(prev_stoppage_data) if prev_stoppage_data is not None else 0,"prev_longest_duration" : prev_longest_duration,"highest_count" : highest_count,"prev_highest_count" : prev_highest_count}

        except Exception as e:
                print(f"Error occurred: {e}")
                mc_data = {'speed': 0,'status': "Offline",'laminate_length': round(laminate_length, 2),'laminate_duration': str(empty_duration),'jamming': True,'pulling': True,'sealing': True,'sealing_time_vertical': 0,'sealing_time_horizontal': 0,'stoppage_time': round(stoppage_time_minute),"longest_duration" : longest_duration,'cld_production': cld_production,'stoppage_count': 0,'outgoing_quality': True,'vertical_pressure':0,'vertical_rear_temp': 0,
                    'horizontal_pressure': 0,'vertical_front_temp': 0,'breakdown_prediction': True,'hopper_level_primary': 0,'horizontal_rear_temp':0,'horizontal_front_temp':0,'hopper_level_secondary': 0,'productivity_prediction': True, 'machine_uptime' : str(machine_uptime),'bboee':round(BBOEE,2),'expected_cld_production':expected_cld,'prev_expected_cld_production':prev_expected_cld,'prev_cld_production':prev_cld_production,'prev_bboee':round(prev_bboee,2),"prev_stoppage_time" : round(prev_stoppage_minutes),"prev_stoppage_count": int(prev_stoppage_data) if prev_stoppage_data is not None else 0,"prev_longest_duration" : prev_longest_duration,"highest_count" : highest_count,"prev_highest_count" : prev_highest_count}
                os._exit(1)

        # Apply overrides to MC25 data
        machine_key = "mc25"
        machine_overrides = self.override_config.get("overrides", {}).get("machines", {}).get(machine_key, {}) if self.override_enabled else {}
        if machine_overrides:
            for key in mc_data.keys():
                mc_data[key] = self.apply_override(key, mc_data[key], machine_overrides, machine_key)

        return mc_data

    def get_machine_26(self, start_time,end_time,shift):
        print("\n---------------------------------------------------")
        print("\n\n machine number : mc26")
        print("\n---------------------------------------------------")

        # Fetch laminate waste data
        self.cursor.execute(self.LAMINATE_LENGTH_QUERY, ('mc26', start_time, end_time))
        laminate_result = self.cursor.fetchall()
        if laminate_result:
            total_waste_amount_cm, empty_duration = laminate_result[0]
            laminate_length = total_waste_amount_cm / 100  # Convert cm to meters
        ...
        # Calculate expected CLD for current shift based on elapsed time
        expected_cld = self.calculate_expected_cld(26, elapsed_seconds)
        print(f"Expected CLD (current shift): {expected_cld}")
        ...
        # Get previous shift expected CLD (fixed for 8 hours)
        prev_expected_cld = self.get_fixed_expected_cld(26)

        # Get previous shift CLD production
        prev_shift = self.get_previous_shift(shift)
        prev_cld_production = self.get_previous_shift_cld("mc26", prev_shift)
        print(f"Previous shift CLD production: {prev_cld_production}")
        print(f"Previous expected CLD: {prev_expected_cld}")
    ...
    def get_machine_27(self, start_time,end_time,shift):
        print("\n---------------------------------------------------")
        print("\n\n machine number : mc27")
        print("\n---------------------------------------------------")

        # Fetch laminate waste data
        self.cursor.execute(self.LAMINATE_LENGTH_QUERY, ('mc27', start_time, end_time))
        laminate_result = self.cursor.fetchall()
        if laminate_result:
            total_waste_amount_cm, empty_duration = laminate_result[0]
            laminate_length = total_waste_amount_cm / 100  # Convert cm to meters
        ...
        # Calculate expected CLD for current shift based on elapsed time
        expected_cld = self.calculate_expected_cld(27, elapsed_seconds)
        print(f"Expected CLD (current shift): {expected_cld}")
        ...
        # Get previous shift expected CLD (fixed for 8 hours)
        prev_expected_cld = self.get_fixed_expected_cld(27)

        # Get previous shift CLD production
        prev_shift = self.get_previous_shift(shift)
        prev_cld_production = self.get_previous_shift_cld("mc27", prev_shift)
        print(f"Previous shift CLD production: {prev_cld_production}")
        print(f"Previous expected CLD: {prev_expected_cld}")
        ...
        try:
            ...
            if avg_data_result:
                ...
                mc_data = {'speed': 100, 'status': status_message, 'laminate_length': round(laminate_length, 2), 'laminate_duration': str(empty_duration), 'jamming': True, 'pulling': True, 'sealing': True, 'sealing_time_vertical': 222, 'sealing_time_horizontal': 240, 'stoppage_time': round(stoppage_time_minute), "longest_duration" : longest_duration,'cld_production': cld_production, 'stoppage_count': int(stoppage_count) if stoppage_count is not None else 0, 'outgoing_quality': True, 'vertical_pressure': round(avg_data[2], 2) if avg_data[2] is not None else 0,
                ...
                'hopper_level_secondary': round(hopper_data[1], 2) if hopper_data[1] is not None else 0, 'productivity_prediction': True,'machine_uptime' : str(machine_uptime),'bboee':round(BBOEE,2),'expected_cld_production':expected_cld,'prev_expected_cld_production':prev_expected_cld,'prev_cld_production':prev_cld_production,'prev_bboee':round(prev_bboee,2),"prev_stoppage_time" : round(prev_stoppage_minutes),"prev_stoppage_count": int(prev_stoppage_data) if prev_stoppage_data is not None else 0,"prev_longest_duration" : prev_longest_duration,"highest_count" : highest_count,"prev_highest_count" : prev_highest_count}
    ...
    def get_machine_28(self, start_time, shift):
        ...
        # Calculate expected CLD for current shift based on elapsed time
        expected_cld = self.calculate_expected_cld(28, elapsed_seconds)
        print(f"Expected CLD (current shift): {expected_cld}")
        ...
        # Get previous shift expected CLD (fixed for 8 hours)
        prev_expected_cld = self.get_fixed_expected_cld(28)

        # Get previous shift CLD production
        prev_shift = self.get_previous_shift(shift)
        prev_cld_production = self.get_previous_shift_cld("mc28", prev_shift)
        print(f"Previous shift CLD production: {prev_cld_production}")
        print(f"Previous expected CLD: {prev_expected_cld}")
            ...
            if avg_data_result:
                ...
                mc_data = {'speed':avg_data[1] if avg_data[1] is not None else 100,'status':status_message,'jamming':True,'pulling':True,'sealing':True,'sealing_time_vertical':231,'sealing_time_horizontal':231,'stoppage_time':round(stoppage_time_minute),"longest_duration" : longest_duration,'cld_production':cld_production,'stoppage_count': int(stoppage_count) if stoppage_count is not None else 0,'outgoing_quality':True,'vertical_rear_temp':round(avg_data[2],2) if avg_data[2] is not None else 0,
                   'vertical_front_temp':round(avg_data[3],2) if avg_data[3] is not None else 0,'breakdown_prediction': True,'horizontal_rear_temp': round(avg_data[4], 2) if avg_data[4] is not None else 0
                    ,'horizontal_front_temp':round(avg_data[5],2) if avg_data[5] is not None else 0,'productivity_prediction': True,'machine_uptime' : str(machine_uptime),'bboee':round(BBOEE,2),'expected_cld_production':expected_cld,'prev_expected_cld_production':prev_expected_cld,'prev_cld_production':prev_cld_production,'prev_bboee':round(prev_bboee,2)
                   ,"prev_stoppage_time" : round(prev_stoppage_minutes),"prev_stoppage_count": int(prev_stoppage_data) if prev_stoppage_data is not None else 0,"prev_longest_duration" : prev_longest_duration,"highest_count" : highest_count,"prev_highest_count" : prev_highest_count}
                ...
                'speed': 0,'status': "Offline",'jamming': True,'pulling': True,'sealing': True,'sealing_time_vertical': 0,'sealing_time_horizontal': 0,'stoppage_time': round(stoppage_time_minute),"longest_duration" : longest_duration,'cld_production':cld_production,'stoppage_count': int(stoppage_count) if stoppage_count is not None else 0,'outgoing_quality': True,'vertical_rear_temp': 0,
                    'vertical_front_temp': 0,'breakdown_prediction': True,'horizontal_rear_temp': 0,'horizontal_front_temp': 0,'productivity_prediction': True,'machine_uptime' : str(machine_uptime),'bboee':round(BBOEE,2),'expected_cld_production':expected_cld,'prev_expected_cld_production':prev_expected_cld,'prev_cld_production':prev_cld_production,'prev_bboee':round(prev_bboee,2),"prev_stoppage_time" : round(prev_stoppage_minutes),"prev_stoppage_count": int(prev_stoppage_data) if prev_stoppage_data is not None else 0,"prev_longest_duration" : prev_longest_duration,"highest_count" : highest_count,"prev_highest_count" : prev_highest_count}
    ...
    def get_machine_29(self, start_time,shift):
        ...
        # Calculate expected CLD for current shift based on elapsed time
        expected_cld = self.calculate_expected_cld(29, elapsed_seconds)
        print(f"Expected CLD (current shift): {expected_cld}")
        ...
        # Get previous shift expected CLD (fixed for 8 hours)
        prev_expected_cld = self.get_fixed_expected_cld(29)

        # Get previous shift CLD production
        prev_shift = self.get_previous_shift(shift)
        prev_cld_production = self.get_previous_shift_cld("mc29", prev_shift)
        print(f"Previous shift CLD production: {prev_cld_production}")
        print(f"Previous expected CLD: {prev_expected_cld}")
        ...
        try:
            ...
            if avg_data_result:
                ...
                mc_data = {'speed':avg_data[1] if avg_data[1] is not None else 100,'status':status_message,'jamming':True,'pulling':True,'sealing':True,'sealing_time_vertical':230,'sealing_time_horizontal':240,'stoppage_time':round(stoppage_time_minute),"longest_duration" : longest_duration,'cld_production':cld_production,'stoppage_count':int(stoppage_count) if stoppage_count is not None else 0,'outgoing_quality':True,'vertical_rear_temp':round(avg_data[2],2) if avg_data[2] is not None else 0,
                   'vertical_front_temp':round(avg_data[3],2) if avg_data[3] is not None else 0,'breakdown_prediction': True,'horizontal_rear_temp': round(avg_data[4], 2) if avg_data[4] is not None else 0,'horizontal_front_temp':round(avg_data[5],2) if avg_data[5] is not None else 0,'productivity_prediction': True,'machine_uptime' : str(machine_uptime),'bboee':round(BBOEE,2),'expected_cld_production':expected_cld,'prev_expected_cld_production':prev_expected_cld,'prev_cld_production':prev_cld_production,'prev_bboee':round(prev_bboee,2)
                   ,"prev_stoppage_time" : round(prev_stoppage_minutes),"prev_stoppage_count": int(prev_stoppage_data) if prev_stoppage_data is not None else 0,"prev_longest_duration" : prev_longest_duration,"highest_count" : highest_count,"prev_highest_count" : prev_highest_count}
    ...
    def get_machine_30(self, start_time, end_time, shift):
        print("\n---------------------------------------------------")
        print("\n\n machine number : mc30")
        print("\n---------------------------------------------------")

        # Fetch laminate waste data
        self.cursor.execute(self.LAMINATE_LENGTH_QUERY, ('mc30', start_time, end_time))
        laminate_result = self.cursor.fetchall()
        if laminate_result:
            total_waste_amount_cm, empty_duration = laminate_result[0]
            laminate_length = total_waste_amount_cm / 100  # Convert cm to meters
        ...
        # Calculate expected CLD for current shift based on elapsed time
        expected_cld = self.calculate_expected_cld(30, elapsed_seconds)
        print(f"Expected CLD (current shift): {expected_cld}")
        ...
        # Get previous shift expected CLD (fixed for 8 hours)
        prev_expected_cld = self.get_fixed_expected_cld(30)

        # Get previous shift CLD production
        prev_shift = self.get_previous_shift(shift)
        prev_cld_production = self.get_previous_shift_cld("mc30", prev_shift)
        print(f"Previous shift CLD production: {prev_cld_production}")
        print(f"Previous expected CLD: {prev_expected_cld}")
        ...
        try:
            ...
            if avg_data_result:
                ...
                mc_data = {'speed': 105, 'status': status_message, 'laminate_length': round(laminate_length, 2), 'laminate_duration': str(empty_duration), 'jamming': True, 'pulling': True, 'sealing': True, 'sealing_time_vertical': 222, 'sealing_time_horizontal': 240, 'stoppage_time': round(stoppage_time_minute), "longest_duration" : longest_duration,'cld_production': cld_production, 'stoppage_count': int(stoppage_count) if stoppage_count is not None else 0, 'outgoing_quality': True, 'vertical_pressure': round(avg_data[2], 2) if avg_data[2] is not None else 0,
                ...
                'horizontal_front_temp': round(avg_data[9], 2) if avg_data[9] is not None else 0, 'hopper_level_secondary': round(hopper_data[1], 2) if hopper_data[1] is not None else 0, 'productivity_prediction': True,'machine_uptime' : str(machine_uptime),'bboee':round(BBOEE,2),'expected_cld_production':expected_cld,'prev_expected_cld_production':prev_expected_cld,'prev_cld_production':prev_cld_production,'prev_bboee':round(prev_bboee,2),"prev_stoppage_time" : round(prev_stoppage_minutes),"prev_stoppage_count": int(prev_stoppage_data) if prev_stoppage_data is not None else 0,"prev_longest_duration" : prev_longest_duration,"highest_count" : highest_count,"prev_highest_count" : prev_highest_count}
    ...
    def fetch_latest_values(self, start_time, end_time,shift):
        try:
            ...
            machines_data = {
                "mc25": self.get_machine_25(start_time, end_time, shift),
                "mc26": self.get_machine_26(start_time, end_time, shift),
                "mc27": self.get_machine_27(start_time, end_time, shift),
                "mc28": self.get_machine_28(start_time, shift),
                "mc29": self.get_machine_29(start_time, shift),
                "mc30": self.get_machine_30(start_time, end_time, shift)
            }

            # Apply overrides to common metrics
            primary_tank = self.apply_override("primary_tank", primary_tank)
            secondary_tank = self.apply_override("secondary_tank", secondary_tank)
            taping_machine = self.apply_override("taping_machine", taping_machine)
            case_erector = self.apply_override("case_erector", case_erector)
            checkweigher = self.apply_override("checkweigher", checkweigher)
    ...
    def fetch_batch_sku_and_shift(self):
        try :
            ...
            if result:
                ...
                # Apply override to batch_sku
                batch_sku = self.apply_override("batch_sku", batch_sku)

                return batch_sku, shift
            else:
                logging.info("No batch_sku and shift found, defaulting to '--'")
                batch_sku = self.apply_override("batch_sku", " ")
                return batch_sku, " "
    ...
    def calculate_ega(self):
        try:
            ...
            # Apply override to EGA
            ega = self.apply_override("ega", ega)
            return ega
        except Exception as e:
    ...
    def calculate_bulk_settling_time(self):
        try:
            ...
            # Apply override to bulk settling time
            bulk_settling_time = self.apply_override("bulk_settling_time", bulk_settling_time)
    ...
    def calculate_cld_production(self,shift):
        try:
            ...
            # Apply override to total CLD production
            cld_production = self.apply_override("cld_production", cld_production)
