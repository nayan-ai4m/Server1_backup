...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("loop4_monitor_new.log"), logging.StreamHandler()],
)
        ...
        if os.path.exists(override_file):
            with open(override_file) as f:
                ...
                if override_config.get("override_enabled", False):
                    logging.warning(
                        "⚠️  OVERRIDE MODE ENABLED - Using hardcoded values from loop4_config_overrides.json"
                    )
...
class Loop4Monitor:
    ...
    def apply_override(
        self, key, calculated_value, override_dict=None, machine_num=None
    ):
        """
        ...
        try:
            if override_dict is None:
                ...
                if machine_num:
                    override_dict = (
                        self.override_config.get("overrides", {})
                        .get("machines", {})
                        .get(machine_num, {})
                    )
                else:
                    override_dict = self.override_config.get("overrides", {}).get(
                        "common_metrics", {}
                    )
            ...
            if (
                ...
                logging.info(
                    f"🔧 Override applied for '{key}' on {machine_num or 'common'}: {override_value}"
                )
    ...
    def calculate_expected_cld(self, machine_number, elapsed_seconds):
        ...
        production_rate = production_rates.get(
            machine_number, 28800 / 600
        )  # Default: 48 seconds per CLD
    ...
    def get_previous_shift_cld(self, machine_key, previous_shift):
        ...
        try:
            self.db.cursor.execute(
                self.GET_PREVIOUS_SHIFT_MACHINES_QUERY, (previous_shift,)
            )
    ...
    def get_machine_25(self, start_time, end_time, shift):
        ...
        stoppage_time, stoppage_count, longest_duration, highest_count = (
            self.write_db.cursor.fetchone()
        )
        ...
        if (
            stoppage_time
            and stoppage_time[0] is not None
            or (prev_stoppage_time and prev_stoppage_time[0] is not None)
        ):
        ...
        # Cap cld_production to not exceed expected_cld
        cld_production = min(cld_production, expected_cld)
        print(f"Capped CLD production: {cld_production}")
        ...
        # Cap prev_cld_production to not exceed prev_expected_cld
        prev_cld_production = min(prev_cld_production, prev_expected_cld)
        ...
        prev_bboee = (
            (prev_cld_production / prev_expected_cld) * 100
            if prev_expected_cld > 0
            else 0
        )
        ...
        try:
            ...
            if avg_data_result:
                ...
                mc_data = {
                    ...
                    "stoppage_time": round(stoppage_time_minute),
                    "longest_duration": longest_duration,
                    "cld_production": cld_production,
                    "stoppage_count": int(stoppage_count)
                    if stoppage_count is not None
                    else 0,
                    "outgoing_quality": True,
                    "vertical_pressure": round(avg_data[2], 2)
                    if avg_data[2] is not None
                    else 0,
                    "vertical_rear_temp": round(avg_data[6], 2)
                    if avg_data[6] is not None
                    else 0,
                    "horizontal_pressure": round(avg_data[3], 2)
                    if avg_data[3] is not None
                    else 0,
                    "vertical_front_temp": round(avg_data[7], 2)
                    if avg_data[7] is not None
                    else 0,
                    "breakdown_prediction": True,
                    "hopper_level_primary": round(hopper_data[0], 2)
                    if hopper_data[0] is not None
                    else 0,
                    "horizontal_rear_temp": round(avg_data[8], 2)
                    if avg_data[8] is not None
                    else 0,
                    "horizontal_front_temp": round(avg_data[9], 2)
                    if avg_data[9] is not None
                    else 0,
                    "hopper_level_secondary": round(hopper_data[1], 2)
                    if hopper_data[1] is not None
                    else 0,
                    "productivity_prediction": True,
                    "machine_uptime": str(machine_uptime),
                    "bboee": round(BBOEE, 2),
                    "expected_cld_production": expected_cld,
                    "prev_expected_cld_production": prev_expected_cld,
                    "prev_cld_production": prev_cld_production,
                    "prev_bboee": round(prev_bboee, 2),
                    "prev_stoppage_time": round(prev_stoppage_minutes),
                    "prev_stoppage_count": int(prev_stoppage_data)
                ...
                mc_data = (
                    self.last_valid_mc25_data
                    if self.last_valid_mc25_data is not None
                    else {
                        "speed": 0,
                        "status": "Offline",
                        "laminate_length": round(laminate_length, 2),
                        ...
                        "pulling": True,
                        "sealing": True,
                        "sealing_time_vertical": 0,
                        "sealing_time_horizontal": 0,
                        "stoppage_time": round(stoppage_time_minute),
                        "longest_duration": longest_duration,
                        "cld_production": cld_production,
                        "stoppage_count": int(stoppage_count)
        ...
        if machine_overrides:
            for key in mc_data.keys():
                mc_data[key] = self.apply_override(
                    key, mc_data[key], machine_overrides, machine_key
                )
    ...
    def get_machine_26(self, start_time, end_time, shift):
        ...
        try:
            ...
            if avg_data_result:
                ...
                mc_data = (
                    self.last_valid_mc26_data
                    if self.last_valid_mc26_data is not None
                    else {
                        "speed": 0,
                        "status": "Offline",
                        "laminate_length": round(laminate_length, 2),
        ...
        try:
            ...
            if avg_data_result:
                ...
                mc_data = {
                    ...
                    "pulling": True,
                    "sealing": True,
                    "sealing_time_vertical": 222,
                    "sealing_time_horizontal": 240,
                    "stoppage_time": round(stoppage_time_minute),
                    "longest_duration": longest_duration,
                    "cld_production": cld_production,
                    "stoppage_count": int(stoppage_count)
                ...
                mc_data = (
                    self.last_valid_mc27_data
                    if self.last_valid_mc27_data is not None
                    else {
                        "speed": 0,
                        "status": "Offline",
                        "laminate_length": round(laminate_length, 2),
    ...
    def get_machine_28(self, start_time, shift):
        ...
        query = self.GET_MACHINE_STATUS_QUERY_28_29.format(
            cld_column=cld_column, mc="mc28"
        )
        ...
        try:
            self.db.cursor.execute(
                self.GET_MACHINE_DATA_QUERY_28_29.format(
                    cld_column=cld_column, mc="mc28"
                )
            )
            avg_data_result = self.db.cursor.fetchall()
            ...
            if avg_data_result:
                ...
                mc_data = {
                    "speed": avg_data[1] if avg_data[1] is not None else 100,
                    "status": status_message,
                    "jamming": True,
                    "pulling": True,
                    "sealing": True,
                    "sealing_time_vertical": 231,
                    "sealing_time_horizontal": 231,
                    "stoppage_time": round(stoppage_time_minute),
                    "longest_duration": longest_duration,
                    "cld_production": cld_production,
                    "stoppage_count": int(stoppage_count)
                    ...
                    "outgoing_quality": True,
                    "vertical_rear_temp": round(avg_data[2], 2)
                    if avg_data[2] is not None
                    else 0,
                    "vertical_front_temp": round(avg_data[3], 2)
                    if avg_data[3] is not None
                    else 0,
                    "breakdown_prediction": True,
                    "horizontal_rear_temp": round(avg_data[4], 2)
                    if avg_data[4] is not None
                    else 0,
                    "horizontal_front_temp": round(avg_data[5], 2)
                    if avg_data[5] is not None
                    else 0,
                    "productivity_prediction": True,
                    "machine_uptime": str(machine_uptime),
                    "bboee": round(BBOEE, 2),
                ...
                mc_data = (
                    self.last_valid_mc28_data
                    if self.last_valid_mc28_data is not None
                    else {
                        "speed": 0,
                        "status": "Offline",
                        "jamming": True,
    ...
    def get_machine_29(self, start_time, shift):
        ...
        try:
            ...
            if avg_data_result:
                ...
                mc_data = (
                    self.last_valid_mc29_data
                    if self.last_valid_mc29_data is not None
                    else {
                        "speed": 0,
                        "status": "Offline",
                        "jamming": True,
    ...
    def get_machine_30(self, start_time, end_time, shift):
        ...
        try:
            ...
            if avg_data_result:
                ...
                mc_data = (
                    self.last_valid_mc30_data
                    if self.last_valid_mc30_data is not None
                    else {
                        "speed": 0,
                        "status": "Offline",
                        "laminate_length": round(laminate_length, 2),
    ...
    def calculate_bulk_settling_time(self):
        try:
            ...
            bulk_settling_time = self.apply_override(
                "bulk_settling_time", bulk_settling_time
            )
    ...
    def calculate_cld_production(self, shift):
        try:
            ...
            for mc_table, column in machines.items():
                query = self.CLD_PRODUCTION_QUERY_TEMPLATE.format(
                    column=column, mc_table=mc_table
                )
                self.db.cursor.execute(query)
                result = self.db.cursor.fetchone()
                ...
                if result:
                    ...
                    if latest_timestamp.date() == today_date:
                        cld_production += (
                            latest_count if latest_count is not None else 0
                        )
                    else:
                        cld_production += 0
    ...
    def update_loop4(self):
        try:
            ...
            (
                ...
                machines_data,
                checkweigher,
            ) = self.fetch_latest_values(start_time, end_time, shift)

            batch_sku, _ = (
                self.fetch_batch_sku_and_shift()
            )  # Ignore database shift, use calculated shift
    ...
    def insert_into_loop4(
        ...
        try:
            ...
            self.db.commit()
            print(
                f"Updated data at {current_time} with bulk settling time {bulk_settling_time}"
            )
