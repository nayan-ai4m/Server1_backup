...
def load_override_config():
    ...
    try:
        override_file = "loop3_config_overrides.json"
        if os.path.exists(override_file):
            with open(override_file) as f:
                override_config = json.load(f)
                if override_config.get("override_enabled", False):
                    logging.warning("⚠️  OVERRIDE MODE ENABLED - Using hardcoded values from loop3_config_overrides.json")
...
class Loop3Monitor:
    ...
    def apply_override(self, key, calculated_value, override_dict=None, machine_num=None):
        ...
        Args:
            ...
            machine_num: Optional machine number (e.g., 'mc17') for machine-specific overrides
    ...
    def calculate_expected_cld(self, machine_number, elapsed_seconds):
        ...
        # MC17-22: 600 CLDs in 8 hours = 48 seconds per CLD
        production_rate = 28800 / 600  # 48 seconds per CLD for all Loop 3 machines
    ...
    def get_fixed_expected_cld(self, machine_number):
        ...
        # All Loop 3 machines (mc17-22) produce 600 CLDs in 8 hours
        return 600
    ...
    def get_machine_data(self, mc, json_key, start_time, end_time, shift):
        ...
        try:
            ...
            expected_cld = self.calculate_expected_cld(int(mc), elapsed_seconds)
            ...
            prev_expected_cld = self.get_fixed_expected_cld(int(mc))

            # Get previous shift CLD production
            prev_shift = self.get_previous_shift(shift)
            prev_cld_production = self.get_previous_shift_cld(f"mc{mc}", prev_shift)
            ...
            # Apply overrides to machine data
            machine_key = f"mc{mc}"
            ...
            if main_data or mid_data:
                ...
                # Apply overrides even for fallback data
                if machine_overrides:
                    for key in mc_data.keys():
                        mc_data[key] = self.apply_override(key, mc_data[key], machine_overrides, machine_key)
            ...
            # Apply overrides even in error case
            machine_key = f"mc{mc}"
            machine_overrides = self.override_config.get("overrides", {}).get("machines", {}).get(machine_key, {}) if self.override_enabled else {}
            if machine_overrides:
                for key in mc_data.keys():
        ...
        try:
            ...
            if result:
                ...
                if isinstance(plant_params, dict):
                    ...
                    # Apply overrides to plant data
                    plantair = self.apply_override("plantair", plantair)
                    planttemperature = self.apply_override("planttemperature", planttemperature)

                    return plantair, planttemperature

            # Apply overrides even for default values
            plantair = self.apply_override("plantair", 0.0)
            planttemperature = self.apply_override("planttemperature", 0.0)
            return plantair, planttemperature
        except Exception as e:
            print("error", e)
    ...
    def fetch_batch_sku_and_shift(self):
        try:
            ...
            return self.apply_override("batch_sku", " "), "A"
        except Exception as e:
    ...
    def calculate_ega(self):
        try:
            ...
            result = self.db.cursor.fetchone()
            ega = result[0] if result and result[0] is not None else 0.0
