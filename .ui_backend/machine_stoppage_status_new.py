...
class MachineStoppageMonitorFromTable:
    ...
    Data Sources:
    - mc17-mc22, mc25-mc27, mc30: Queries machine_stoppages table (row-per-stoppage)
    - mc28, mc29: Fetches from machine_stoppage table (pre-aggregated JSON)
    """
    ...
    def close_db(self):
        for cursor, conn in [
            (self.read_cursor, self.read_conn),
            (self.write_cursor, self.write_conn),
        ]:
            if cursor:
                cursor.close()
            if conn:
    ...
    def get_stoppage_count(self, mc, start_time, end_time):
        try:
            self.read_cursor.execute(
                self.GET_STOPPAGE_COUNT_FROM_TABLE, (mc, start_time, end_time)
            )
    ...
    def get_stoppage_time(self, mc, start_time, end_time):
        try:
            self.read_cursor.execute(
                self.GET_TOTAL_STOPPAGE_TIME_FROM_TABLE, (mc, start_time, end_time)
            )
            result = self.read_cursor.fetchone()
    ...
    def get_top_3_longest_durations(self, mc, start_time, end_time):
        try:
            self.read_cursor.execute(
                self.GET_TOP_3_DURATIONS_FROM_TABLE, (mc, start_time, end_time)
            )
    ...
    def get_stoppage_reasons_with_count(self, mc, start_time, end_time):
        try:
            self.read_cursor.execute(
                self.GET_REASON_FREQUENCY_FROM_TABLE, (mc, start_time, end_time)
            )
            results = self.read_cursor.fetchall()
    ...
    def get_machine_data_from_machine_stoppage(self, mc, shift):
        """
        Fetches pre-computed machine data from machine_stoppage table for mc28 and mc29.
        Returns the JSON data directly as it's already aggregated.
        """
        try:
            query = f"""
                SELECT {mc}
                FROM machine_stoppage
                WHERE shift = %s
                ORDER BY timestamp DESC
                LIMIT 1
            """
            self.read_cursor.execute(query, (shift,))
            result = self.read_cursor.fetchone()

            if result and result[0]:
                # Data is already in JSON format, parse and return it
                data = result[0] if isinstance(result[0], dict) else json.loads(result[0])
                self.logger.info(f"Fetched pre-computed data for {mc} from machine_stoppage table")
                return data
            else:
                # No data found, return default structure
                self.logger.warning(f"No data found in machine_stoppage for {mc}, shift {shift}")
                return {
                    "stoppage_count": 0,
                    "stoppage_time": 0,
            ...
            error_msg = f"Error fetching data from machine_stoppage for {mc}: {e}"
            self.error_logger.error(error_msg)
            self.error_logger.error(traceback.format_exc())
    ...
    def insert_shift_row_if_missing(self):
        ...
        shift, shift_name, start_time, end_time = self.get_shift_and_time_range(
            current_time
        )
        row_ts = start_time
        try:
            ...
            if self.write_cursor.fetchone():
                self.logger.info(
                    f"Row already exists for ts={row_ts}, shift={shift}. Skipping insert."
                )
                return
            defaults = [self._default_json() for _ in range(12)]
            self.write_cursor.execute(self.INSERT_SHIFT_ROW, (row_ts, shift, *defaults))
    ...
    def update_machine_data(self):
        ...
        row_ts = start_time
        self.logger.info(
            f"Updating shift {shift} ({shift_name}) | Range: {start_time} → {end_time}"
        )
        all_machines = [
            "mc17",
            "mc18",
        ...
        try:
            for mc in all_machines:
                # For mc28 and mc29, fetch from machine_stoppage table directly
                if mc in ['mc28', 'mc29']:
                    data = self.get_machine_data_from_machine_stoppage(mc, shift)
                    machine_data[mc] = json.dumps(data)
                else:
                    # For all other machines, use the existing logic from machine_stoppages table
                    stoppage_count = self.get_stoppage_count(mc, start_time, end_time)
                    stoppage_time = self.get_stoppage_time(mc, start_time, end_time)
                    top_3_durations = self.get_top_3_longest_durations(
                        mc, start_time, end_time
                    )
                    stoppage_reasons = self.get_stoppage_reasons_with_count(
                        mc, start_time, end_time
                    )
                    updated_durations = self.update_top_3_durations(top_3_durations)
                    updated_highest_counts = self.update_highest_count_reasons(
                        stoppage_reasons, top_3_durations
                    )
            ...
            print("\n" + "=" * 80)
            print(
                f"SHIFT: {shift} ({shift_name}) | TIME: {current_time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    def synchronize_reasons_for_single_stoppage(
        self, highest_counts, durations, stoppage_count
    ):
        if stoppage_count == 1:
    ...
    def schedule_shift_inserts(self):
        ...
        for shift_config in self.config.get("shifts", []):
            ...
            schedule.every().day.at(f"{start_hour:02d}:00").do(
                self.insert_shift_row_if_missing
            )
            self.logger.info(
                f" Scheduled INSERT for {shift_name} ({shift}) at {start_hour:02d}:00 daily"
            )
