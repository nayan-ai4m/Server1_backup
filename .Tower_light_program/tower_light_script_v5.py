...
class MachineMonitor:
    ...
    def get_active_count(self):
        ...
        Uses SQL to count active TPs directly, returning only an integer.
        This avoids fetching the large JSON blob which can cause MTU issues.
        """
        try:
            conn = self.get_connection()
            cur = conn.cursor()
            
            # Set statement timeout to 10 seconds to prevent indefinite hangs
            cur.execute("SET statement_timeout = '10s';")

            # Count active TPs directly in SQL (excluding TP01)
            # This returns just an integer instead of the full JSON
            query = f"""
                SELECT (
                    SELECT COUNT(*)
                    FROM jsonb_each(COALESCE(machine_status->'active_tp', '{{}}')) AS x(key, value)
                    WHERE key != 'TP01'
                ) as active_count
            ...
            return result[0]  # Just an integer now!
