...
class ThresholdMonitor:
    ...
    def update_pressure_spread(self, spread_value):
        ...
        try:
            query = """
                UPDATE mc17_autoencoder
                SET pressure_spread = %s
                WHERE timestamp = (
                    SELECT timestamp FROM mc17_autoencoder
                    ORDER BY timestamp DESC
                    LIMIT 1
                )
            """
            ...
            self.conn.commit()
            print(f"Updated mc17_autoencoder with spread = {spread_value:.6f}")
        except Exception as e:
    ...
    def run(self):
        ...
        while self.running:
            ...
            if (2 * std_dev > 0.049) and self.wrinkle_flag == False:
                ...
                query = """UPDATE mc17_tp_status SET tp63 = '{}' """.format(json.dumps(data))
                print(query)
