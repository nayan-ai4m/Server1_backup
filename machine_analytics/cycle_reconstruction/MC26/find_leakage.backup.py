...
class ThresholdMonitor:
    ...
    def query_and_analyze(self):
        ...
        try:
            ...
            setpoint_query = """ select timestamp,(mc26->'HMI_Hor_Seal_Front_27'->>'SetValue')::float as front_value,(mc26->'HMI_Hor_Seal_Rear_28'->>'SetValue')::float as rear_setvalue  from loop3_checkpoints where mc26 != 'null' order by timestamp desc limit 1;"""
            ...
            for col_name, result in results.items():
                ...
                if result['exceeds_60_percent'] and self.flag_status[result['idx']] == False:
                    ...
                    if col_name == 'mae_seal_pressure':
                        ...
                        #query = """UPDATE mc26_tp_status SET tp03 = '{}' """.format(data)
                        ...
                        if col_name == 'mae_seal_pressure':
                            if result['vector']:
                                query = """UPDATE mc26_tp_status SET tp62 = '{}' """.format(data)
                                self.event_cursor.execute(query)
                                self.event_conn.commit()
                                self.insert_event()
                    ...
                    if col_name == 'mae_rear_temp' or col_name == 'mae_front_temp':
                        ...
                        if col_name == 'mae_front_temp':
                            if setpoint_front_temp > front_temp:
                                ...
                                self.event_conn.commit()
                                self.insert_event()
                                data = {"machine":26,"type":"front_temp","setpoint":setpoint_front_temp,"current":front_temp,"timestamp":str(datetime.datetime.now()),"alert":tpdata}
                                self.producer.send(self.topic,value=data)
                        ...
                        if col_name == 'mae_rear_temp':
                            ...
                            if setpoint_rear_temp > rear_temp:
                                ...
                                self.event_conn.commit()
                                self.insert_event()
                                data = {"machine":26,"type":"rear_temp","setpoint":setpoint_rear_temp,"current":rear_temp,"timestamp":str(datetime.datetime.now()),"alert":tpdata}
                                self.producer.send(self.topic,value=data)
                            else :
                                print("inreased")
                else:
