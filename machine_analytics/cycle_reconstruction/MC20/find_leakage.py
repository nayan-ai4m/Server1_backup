...
class ThresholdMonitor:
    ...
    def format_event(self, is_state):
        return {
            ...
            "filepath": "http://192.168.0.158:8015/mc20_detection_2042302886.png",
            "color_code": 1,
            "machine_part": "plc"
        }
    ...
    def disconnect(self):
        ...
        try:
            if self.cursor and not self.cursor.closed:
                self.cursor.close()
            if self.raw_cursor and not self.raw_cursor.closed:
                self.raw_cursor.close()
            if self.event_cursor and not self.event_cursor.closed:
                self.event_cursor.close()
                
            if self.conn and not self.conn.closed:
                self.conn.close()
            if self.raw_conn and not self.raw_conn.closed:
                self.raw_conn.close()
            if self.event_conn and not self.event_conn.closed:
    ...
    def ensure_connection(self):
        """Ensure all database connections are alive."""
        try:
            # Test each connection and cursor
            if not self.conn or self.conn.closed or not self.cursor or self.cursor.closed:
                print("Reconnecting main database...")
                if self.cursor and not self.cursor.closed:
                    self.cursor.close()
                if self.conn and not self.conn.closed:
                    self.conn.close()
            ...
            if not self.raw_conn or self.raw_conn.closed or not self.raw_cursor or self.raw_cursor.closed:
                print("Reconnecting raw database...")
                if self.raw_cursor and not self.raw_cursor.closed:
                    self.raw_cursor.close()
                if self.raw_conn and not self.raw_conn.closed:
                    self.raw_conn.close()
            ...
            if not self.event_conn or self.event_conn.closed or not self.event_cursor or self.event_cursor.closed:
                print("Reconnecting event database...")
                if self.event_cursor and not self.event_cursor.closed:
                    self.event_cursor.close()
                if self.event_conn and not self.event_conn.closed:
                    self.event_conn.close()
    ...
    def check_tp01_status(self):
        ...
        try:
            ...
            query = """SELECT tp01 FROM mc20_tp_status;"""
    ...
    def query_and_analyze(self):
        ...
        try:
            # Ensure connections are valid before querying
            self.ensure_connection()
            ...
            LIMIT 10;
            """
            current_query = """select timestamp,hor_sealer_front_1_temp as front_current_value,hor_sealer_rear_1_temp as rear_current_value from mc20 order by timestamp desc limit 1;"""
            setpoint_query = """ select timestamp,(mc20->'HMI_Ver_Seal_Rear_28'->>'SetValue')::float as front_value,(mc20->'HMI_Ver_Seal_Rear_27'->>'SetValue')::float as rear_setvalue ,(mc20->'HMI_Hor_Sealer_Strk_1')::float as stroke1 , (mc20->'HMI_Hor_Sealer_Strk_2')::float as stroke2 from loop3_checkpoints where mc20 != 'null' order by timestamp desc limit 1;"""
            ...
            for col_name, result in results.items():
                ...
                if result['exceeds_60_percent'] and self.flag_status[result['idx']] == False and self.check_tp01_status():
                    if col_name == 'mae_seal_pressure':
                        query = """select AVG(hor_pressure) from (select hor_pressure from mc20_short_data order by timestamp desc limit 100) as latest_values;"""
                        self.cursor.execute(query)
                        ...
                        if col_name == 'mae_seal_pressure':
                            if result['vector']:
                                ...
                                query = """UPDATE mc20_tp_status SET tp62 = %s"""
                                self.event_cursor.execute(query, (tpdata,))
                                self.event_conn.commit()
                                if self.check_tp01_status():  # Additional check before insert
                                    self.insert_event()
                                data = {"machine": 20, "type": "pressure", "setpoint": setpoint_front_temp, "current": front_temp, "timestamp": str(datetime.datetime.now())}
                                self.producer.send(self.topic, value=data)
                                data = {"machine": 20, "type": "pressure", "setpoints": {'s1': 54.0, 's2': 3, 'front': 154, 'rear': 154}, "current": {'front': 154, 'rear': 154, 'pressure': 1.83}, "timestamp": str(datetime.datetime.now()), "alert": tpdata}
                                self.producer.send(self.topic, value=data)
                                print(data)
                    if col_name == 'mae_rear_temp' or col_name == 'mae_front_temp':
                        ...
                        if col_name == 'mae_front_temp':
                            if setpoint_front_temp > front_temp:
                                print("decreased")
                                tpdata = json.dumps(self.format_event(True))
                                query = """UPDATE mc20_tp_status SET tp61 = %s"""
                                self.event_cursor.execute(query, (tpdata,))
                                self.event_conn.commit()
                                if self.check_tp01_status():  # Additional check before insert
                                    self.insert_event()
                                data = {"machine": 20, "type": "front_temp", "setpoints": {'s1': stroke1, 's2': stroke2, 'front': setpoint_front_temp, 'rear': setpoint_rear_temp}, "current": {'front': front_temp, 'rear': rear_temp}, "timestamp": str(datetime.datetime.now()), "alert": tpdata}
                                self.producer.send(self.topic, value=data)
                            else:
                                print("increased")
                        if col_name == 'mae_rear_temp':
                            ...
                            if setpoint_rear_temp == rear_temp:
                                ...
                                if self.check_tp01_status():  # Additional check before insert
                                    self.insert_event()
                                data = {"machine": 20, "type": "rear_temp", "setpoints": {'s1': stroke1, 's2': stroke2, 'front': setpoint_front_temp, 'rear': setpoint_rear_temp}, "current": {'front': front_temp, 'rear': rear_temp}, "timestamp": str(datetime.datetime.now()), "alert": tpdata}
                                self.producer.send(self.topic, value=data)
                            else:
                                print("inreased")
                else:
                    if result['exceeds_60_percent'] == False and self.check_tp01_status():
                        if self.flag_status[result['idx']] == True:
                            ...
                            query = """UPDATE mc20_tp_status SET tp61 = 'null'"""
                            self.event_cursor.execute(query)
                            self.event_conn.commit()
