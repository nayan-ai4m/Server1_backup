...
class ThresholdMonitor:
    ...
    def insert_event(self ) :
        ...
        event_id = self.generate_event_id("mc17",'leakage', datetime.datetime.now())
        machine_name = 'mc17'
    ...
    def query_and_analyze(self):
        ...
        try:
            ...
            FROM mc17_autoencoder
            ORDER BY timestamp DESC
            LIMIT 10;
            """
            current_query = """select timestamp,hor_sealer_front_1_temp as front_current_value,hor_sealer_rear_1_temp as rear_current_value from mc17 order by timestamp desc limit 1;"""
            setpoint_query = """ select timestamp,(mc17->'HMI_Hor_Seal_Front_27'->>'SetValue')::float as front_value,(mc17->'HMI_Hor_Seal_Rear_28'->>'SetValue')::float as rear_setvalue ,(mc17->'HMI_Hor_Sealer_Strk_1')::float as stroke1 , (mc17->'HMI_Hor_Sealer_Strk_2')::float as stroke2 from loop3_checkpoints where mc17 != 'null' order by timestamp desc limit 1;"""
            # Execute query and fetch results
            self.cursor.execute(query)
            rows = self.cursor.fetchall()
            total_rows = len(rows)

            # Print the queried rows
            ...
            """
            self.event_cursor.execute(current_query)
            db_rows = self.event_cursor.fetchone()
            (front_temp,rear_temp) = db_rows[1:]
            self.event_cursor.execute(setpoint_query)
            db_rows = self.event_cursor.fetchone()
            (setpoint_front_temp,setpoint_rear_temp) = db_rows[1:]
            print(front_temp,rear_temp,setpoint_front_temp,setpoint_rear_temp)
            """
            ...
            for col_name, result in results.items():
                ...
                if result['exceeds_60_percent'] and self.flag_status[result['idx']] == False:

                    if col_name == 'mae_seal_pressure':
                        query = """select AVG(hor_pressure) from (select hor_pressure from mc17_short_data order by timestamp desc limit 100) as latest_values;"""
                        self.cursor.execute(query)
                        ...
                        #query = """UPDATE mc17_tp_status SET tp03 = '{}' """.format(data)
                        #self.event_cursor.execute(query)
                        #self.event_conn.commit()
                        #self.leakage_flag = True
                        self.flag_status[result['idx']] = True
                        if col_name == 'mae_seal_pressure':
                            if result['vector']:
                                tpdata = json.dumps(self.format_event(True))
                                query = """UPDATE mc17_tp_status SET tp62 = '{}' """.format(tpdata)
                                self.event_cursor.execute(query)
                                self.event_conn.commit()
                                self.insert_event()
                                data = {"machine":17,"type":"pressure","setpoint":setpoint_front_temp,"current":front_temp,"timestamp":str(datetime.datetime.now())}
                                self.producer.send(self.topic,value=data)
                                data = {"machine":17,"type":"pressure","setpoints":{'s1':54.0,'s2':3,'front':154,'rear':154},"current":{'front':154,'rear':154,'pressure':1.83},"timestamp":str(datetime.datetime.now()),"alert":tpdata}
                                self.producer.send(self.topic,value=data)
                                print(data)
                    if col_name == 'mae_rear_temp' or col_name == 'mae_front_temp':
                        self.event_cursor.execute(current_query)
                        rows = self.event_cursor.fetchone()
                        (front_temp,rear_temp) = rows[1:]
                        self.event_cursor.execute(setpoint_query)
                        rows = self.event_cursor.fetchone()
                        (setpoint_front_temp,setpoint_rear_temp,stroke1,stroke2) = rows[1:]
                        ...
                        if col_name == 'mae_front_temp':
                            if setpoint_front_temp > front_temp:
                                print("decreased")
                                tpdata = json.dumps(self.format_event(True))
                                query = """UPDATE mc17_tp_status SET tp61 = '{}' """.format(tpdata)
                                self.event_cursor.execute(query)
                                self.event_conn.commit()
                                self.insert_event()
                                data = {"machine":17,"type":"front_temp","setpoints":{'s1':stroke1,'s2':stroke2,'front':setpoint_front_temp,'rear':setpoint_rear_temp},"current":{'front':front_temp,'rear':rear_temp},"timestamp":str(datetime.datetime.now()),"alert":tpdata}
                                self.producer.send(self.topic,value=data)
                        ...
                        if col_name == 'mae_rear_temp':
                            ...
                            if setpoint_rear_temp == rear_temp:
                                ...
                                self.event_conn.commit()
                                self.insert_event()
                                data = {"machine":17,"type":"rear_temp","setpoints":{'s1':stroke1,'s2':stroke2,'front':setpoint_front_temp,'rear':setpoint_rear_temp},"current":{'front':front_temp,'rear':rear_temp},"timestamp":str(datetime.datetime.now()),"alert":tpdata}
                                self.producer.send(self.topic,value=data)
                            else :
                                print("inreased")
                else:
                    if result['exceeds_60_percent'] == False:
                        if self.flag_status[result['idx']] == True:
                            self.flag_status[result['idx']] = False
                            query = """UPDATE mc17_tp_status SET tp61 = 'null' """
...
if __name__ == "__main__":
    ...
    monitor = ThresholdMonitor(db_params, table_name='mc17_short_data', query_interval=3)
    monitor.run()
