...
MAE_table_name = 'mc27_autoencoder'
...
def get_latest_spare1():
    ...
    cursor = conn.cursor()
    query = "SELECT spare1 FROM mc27_short_data ORDER BY timestamp DESC LIMIT 1;"
...
def fetch_data_by_spare1(spare1):
    ...
    short_columns = ['timestamp', 'cam_position', 'hor_sealer_current', 
    ...
    df_short['hor_sealer_front_1_temp'] = front_temp
    df_short['hor_sealer_rear_1_temp'] = rear_temp

    print(f"Data fetched for spare1: {spare1}, rows: {len(df_short)} (with latest temps from mc27_mid)")
    return df_short
...
def reconstruct_tensor(input_array):                                                      # Modify directory path
    ...
    response = client.infer(model_name="reconstruction_mc27", inputs=[inputs], outputs=[outputs])
