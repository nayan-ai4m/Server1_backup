...
MAE_table_name = 'mc22_autoencoder'
...
def fetch_data_by_spare1(spare1):
    ...
    print(f"Data fetched for spare1: {spare1}, rows: {len(df_short)} (with latest temps from mc22_mid)")
    return df_short
...
def reconstruct_tensor(input_array):                                                      # Modify directory path
    ...
    response = client.infer(model_name="reconstruction_mc22", inputs=[inputs], outputs=[outputs])
