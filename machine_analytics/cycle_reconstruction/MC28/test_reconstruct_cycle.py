...
MAE_table_name = 'mc28_autoencoder'
...
def get_latest_spare1():
    ...
    cursor = conn.cursor()
    query = "SELECT spare1 FROM mc28_short_data ORDER BY timestamp DESC LIMIT 1;"
...
def fetch_data_by_spare1(spare1):
    ...
    print(f"Data fetched for spare1: {spare1}, rows: {len(df_short)} (with latest temps from mc28_mid)")
    return df_short
...
def reconstruct_tensor(input_array):                                                      # Modify directory path
    ...
    response = client.infer(model_name="reconstruction_mc28", inputs=[inputs], outputs=[outputs])
