...
def setup_configuration_table():
    ...
    try:
        ...
        CREATE TABLE IF NOT EXISTS machine_sync_config (
            ...
            -- For MC25/MC26/MC27/MC30 machines
...
def insert_default_sync_configurations():
    ...
    try:
        ...
        # Advanced machines (MC25, MC26, MC27, MC30)
        advanced_configs = [
            ('MC25', 'mc25_short_data', 'sync_data_25', 'cycle_id', 'status'),
            ('MC26', 'mc26_short_data', 'sync_data_26', 'cycle_id', 'status'),
            ('MC27', 'mc27_short_data', 'sync_data_27', 'cycle_id', 'status'),
            ('MC30', 'mc30_short_data', 'sync_data_30', 'cycle_id', 'status'),
        ]
        for machine_id, source_table, sync_table, cycle_col, status_col in advanced_configs:
            torque_col = 'hor_torque' if machine_id == 'MC27' else 'hor_tq'
            cursor.execute("""
            INSERT INTO machine_sync_config
            (machine_id, source_table, sync_table, cycle_column, status_column,
             horizontal_sealer_col, vertical_sealer_col, piston_right_col,
             rotary_valve_col, puller_tq_col, torque_invert, machine_type,
             enable_plots, current_scale_factor)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (machine_id) DO NOTHING;
            """, (
                machine_id, source_table, sync_table, cycle_col, status_col,
...
def fetch_cycle_data(config, cycle_id, logger):
    ...
    try:
        ...
        if config['machine_type'] == 'standard':
            ...
            # Advanced machine columns (MC25/MC26/MC27/MC30)
...
def insert_sync_data(config, cycle_id, vs_hs_sync, vs_hs_avg, vs_rv_sync, vs_rv_avg,
    ...
    """Insert sync data – ALWAYS into the `spare1` column."""
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cur = conn.cursor()

        # ALWAYS use spare1 – even for advanced machines
        cycle_col = 'spare1'

        query = f"""
        INSERT INTO {config['sync_table']}
        (spare1,
         vertical_horizontal_sync, vertical_horizontal_sync_avg,
         vertical_rotational_sync, vertical_rotational_sync_avg,
         filling_rotational_sync, filling_rotational_sync_avg,
         pulling_horizontal_sync, pulling_horizontal_sync_avg,
         timestamp)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);
        """
        cur.execute(query, (
            int(cycle_id),                     # <-- value from cycle_id column
            float(vs_hs_sync), float(vs_hs_avg),
            float(vs_rv_sync), float(vs_rv_avg),
            float(fp_rv_sync), float(fp_rv_avg),
            float(ps_hs_sync), float(ps_hs_avg),
            timestamp
        ))
        conn.commit()
        logger.info(f"INSERTED {config['sync_table']} → spare1={cycle_id} | "
                   f"VS-HS={vs_hs_avg:.1f}% PS-HS={ps_hs_avg:.1f}%")
        return True
    except Exception as e:
...
def process_advanced_machine(config, df, cycle_id, avg_buffers, sync_metrics, logger):
    """Process synchronization for advanced machines (MC25/MC26/MC27/MC30)."""
    status = df['status'].iloc[0]
    if not bool(status):
        logger.info(f"Machine stopped for cycle {cycle_id}")
        return
...
def main():
    ...
    if args.all:
        ...
        print(" python sync_unified.py --machine MC27 # Monitor MC27")
