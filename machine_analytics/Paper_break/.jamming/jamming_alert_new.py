...
class JAMMING_TP:
    def __init__(self):
        ...
        self.conn_tp = psycopg2.connect(
            host="192.168.1.168", dbname="hul", user="postgres", password="ai4m2024"
        )
        ...
        for machine in self.machines:
            ...
            self.ensure_tp_column_exists(tp_table, "tp09")
            self.ensure_tp_column_exists(tp_table, "tp10")
    ...
    def ensure_tp_column_exists(self, tp_table, tp_column):
        ...
        Args:
            ...
            tp_column (str): Name of the column to check or add (e.g., 'tp09').
        """
        try:
            ...
            if not column_exists:
                print(
                    f"Column '{tp_column}' does not exist in table '{tp_table}'. Creating it..."
                )
                ...
                print(
                    f"Column '{tp_column}' already exists in table '{tp_table}'. No action needed."
                )
        except psycopg2.Error as e:
            print(
                f"Error checking/creating column '{tp_column}' in table '{tp_table}': {e}"
            )
    ...
    def run(self):
        print(
            "Started Code to check Laminate Jamming for machines: "
            + ", ".join([m.upper() for m in self.machines])
        )
            ...
            try:
                for machine in self.machines:
                    ...
                    jamming_table = (
                        "jamming_data"
                        if machine == "mc17"
                        else f"jamming_data_{machine[2:]}"
                    )
                    ...
                    if jamming is not None:
                        # Check what conditions are present in the jamming reason
                        has_baseline = "Baseline avg" in jamming
                        has_rate = "Rate of change" in jamming
                        # Check for absolute thresholds (2.0A or 3.8A)
                        has_absolute = "Current avg > 2.0A" in jamming or "Current avg > 3.8A" in jamming

                        # Ignore standalone rate of change alerts
                        if has_rate and not has_baseline and not has_absolute:
                            is_problem = None
                            flag = None
                        # ABRUPT: Absolute max threshold combined with baseline or rate
                        # Examples: "Baseline & Current avg > 2.0A", "Rate & Current avg > 2.0A", or all three
                        elif has_absolute and (has_baseline or has_rate):
                            is_problem = True
                            flag = "abrupt"
                        # CONTINUOUS: Baseline only, absolute only, or baseline + rate (without absolute)
                        # Examples: "Baseline only", "Current avg > 2.0A only", "Baseline & Rate"
                        elif has_baseline or has_absolute:
                            is_problem = True
                            flag = "continuous"
                        else:
                            is_problem = None
                    ...
                    is_new = current_timestamp is not None and (
                        refer_timestamp is None or current_timestamp != refer_timestamp
                    )

                    if is_new and flag is not None:
                        # Check if reason is different or if 5 minutes have passed
                        elapsed = (
                            datetime.now() - check_time
                        ...
                        if prev_reason != jamming or elapsed >= timedelta(minutes=5):
                            ...
                            # Update tp_status
                            tp_column = "tp09" if flag == "continuous" else "tp10"
                            check_tp_exists_query = (
                                f"SELECT {tp_column} FROM {tp_table};"
                            )
                            update_tp_query = f"UPDATE {tp_table} SET {tp_column} = %s;"
                            insert_tp_query = (
                                f"INSERT INTO {tp_table} ({tp_column}) VALUES (%s);"
                            )
                            ...
                            if exists:
                                print(
                                    f"Updating {tp_column} for {machine} with event data: {event_data}"
                                )
                                self.cursor_tp.execute(
                                    update_tp_query, (json.dumps(event_data),)
                                )
                            else:
                                print(
                                    f"Inserting new {tp_column} for {machine} with event data: {event_data}"
                                )
                                self.cursor_tp.execute(
                                    insert_tp_query, (json.dumps(event_data),)
                                )
                    ...
                    if prev_flag is not None:
                        ...
                        if elapsed >= timedelta(minutes=5):
                            ...
                            event_data = self.format_event_data(
                                is_problem=False,
                                last_active_timestamp=md["last_active_timestamp"],
                            )

                            tp_table = f"{machine}_tp_status"
                            tp_column = "tp09" if prev_flag == "continuous" else "tp10"
                            check_tp_exists_query = (
                                f"SELECT {tp_column} FROM {tp_table};"
                            )
                            update_tp_query = f"UPDATE {tp_table} SET {tp_column} = %s;"
                            ...
                            if exists:
                                print(
                                    f"Clearing previous event in {tp_column} for {machine} with event data: {event_data}"
                                )
                                self.cursor_tp.execute(
                                    update_tp_query, (json.dumps(event_data),)
                                )
