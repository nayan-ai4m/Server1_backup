#!/usr/bin/env python3
"""
S2 Quality Monitor for MC27 & MC30
Monitors seal quality (S2) and generates alerts based on instant load and heat energy
"""
...
def load_config(machine_no: str):
    ...
    try:
        ...
        log_error(f"Config file {filename} not found", machine=f"{machine_no}")
        return None
    except Exception as e:
        log_error(f"Error loading config file {filename}", e, f"{machine_no}")
        return None

# ===== DB FETCH =====
def fetch_data_from_postgres(machine_id):
    """Fetch instant load and temperature data from postgres"""
    ...
    # --- Fetch instant load data from fast table ---
    conn_fast = psycopg2.connect(**READ_DB_CONFIG)
    query_fast = f"""
    SELECT timestamp, 
           horizontal_instant_load as instant_load,
           cam_position,
           status,
           cycle_id
...
# ===== S2 CALCULATION =====
def calculate_s2(heat_energy, instant_load, cfg):
    """Calculate S2 seal quality metric"""
    if heat_energy is None or instant_load is None:
        return None
    
    # S2 coefficients
    a = cfg.get("seal_a", 0.35)  # Coefficient for heat
    b = cfg.get("seal_b", 0.4)   # Coefficient for instant load
    c = cfg.get("seal_c", 0.25)  # Coefficient for sealing time
    
    # Targets
    heat_target = cfg.get("HEAT_TARGET", 315.0)
    instant_load_target = cfg.get("INSTANT_LOAD_TARGET", 2.5)
    ...
    S2 = (a * (heat_energy / heat_target) +
          b * (instant_load / instant_load_target) +
...
def process_data(df, cfg):
    ...
    # Calculate average instant load for cam positions 170-200
    instant_load_per_window = (
        df[(df['cam_position'] >= 170) & (df['cam_position'] <= 200)]
        .groupby('time_window')['instant_load']
        .mean()
        .reset_index(name='instant_load_avg')
    )
    ...
    window_data = window_data.merge(instant_load_per_window, on='time_window', how='left')
    ...
    window_data['s2'] = window_data.apply(
        lambda row: calculate_s2(row['heat_energy'], row['instant_load_avg'], cfg), axis=1
    )
...
def send_low_instant_load_event(machine_id, ts, suggestions):
    """Send alert for low instant load"""
    try:
        conn = psycopg2.connect(
            dbname="hul", user="postgres", password="ai4m2024",
            host="192.168.1.168", port="5432"
        ...
        tp = 62  # TP for instant load issues
        tpdata = json.dumps(format_event(True))
        cur.execute(f"UPDATE mc{machine_id}_tp_status SET tp{tp} = %s", (tpdata,))
        conn.commit()
        ...
        message = {
            "suggestion": {
                "HMI_Hor_Sealer_Strk_1": s1,
                "HMI_Hor_Sealer_Strk_2": s2,
                "text": f"Low Instant Load: Change Stroke 1 to {s1}, Stroke 2 to {s2}"
            }
        }
        ...
        log_event(f"Low instant load alert sent, suggestion applied", ts, f"mc{machine_id}")
...
def main_loop(machine_id, refresh_sec=2):
    ...
    # Thresholds
    S2_THRESHOLD = 0.75
    MIN_INSTANT_LOAD = 1.5
    MIN_HEAT = 135  # Minimum temperature translates to heat energy
    ...
    while True:
        try:
            ...
            if cfg is None:
                print(f"[MC{machine_id}] Config load failed, retrying...")
            ...
            if config_changed and previous_config is not None:
                ...
                if 'error' in suggestions:
                    log_error(f"Failed to get suggestions: {suggestions['error']}", machine=f"mc{machine_id}")
                else:
            ...
            if machine_running and startup_delay_complete:
                ...
                if not window_df.empty:
                    ...
                    latest_instant_load = latest_row['instant_load_avg']
                    ...
                    if 'error' in suggestions:
                        log_error(f"Failed to get suggestions: {suggestions['error']}", machine=f"mc{machine_id}")
                        suggestions = last_suggestion if last_suggestion else {
                            "suggested_temp": 0.0,
                            "suggested_s1": 0.0,
                            "suggested_s2": 0.0
                        }
                    ...
                    if pd.notna(latest_s2):
                        print(f"[MC{machine_id}] [{latest_machine_ts}] "
                              ...
                              f"Instant Load: {latest_instant_load:.2f}, "
                        ...
                        if latest_s2 < S2_THRESHOLD:
                            ...
                            instant_load_ratio = latest_instant_load / cfg.get("INSTANT_LOAD_TARGET", 2.5)
                            
                            # Find which is further from target
                            if heat_ratio < instant_load_ratio:
                                # Heat is the limiting factor
                                event_counters["low_heat"] += 1
                                event_counters["low_instant_load"] = 0  # Reset instant load counter
                                ...
                                # Instant load is the limiting factor
                                event_counters["low_instant_load"] += 1
                                event_counters["low_heat"] = 0  # Reset heat counter
                                
                                print(f"[MC{machine_id}] Low S2 due to instant load: {latest_instant_load:.2f} "
                                      f"(ratio: {instant_load_ratio:.3f})")
                                
                                if event_counters["low_instant_load"] >= 5:
                                    log_event(f"Low instant load detected (5 consecutive), sending alert",
                                             latest_machine_ts, f"mc{machine_id}")
                                    send_low_instant_load_event(machine_id, latest_machine_ts, suggestions)
                                    event_counters["low_instant_load"] = 0  # Reset after alert
                        else:
                    ...
                    if last_suggestion != suggestions and 'error' not in suggestions:
                        log_event(
...
if __name__ == "__main__":
    machine_ids = [27, 30]  # Monitor MC27 and MC30
    processes = []
    
    print("Starting S2 Quality Monitor for MC27 and MC30...")
