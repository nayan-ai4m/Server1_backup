        ...
        if machine_running and not (np.isnan(latest_heat) or np.isnan(latest_pressure)):
            ...
            if True:
                if latest_heat < heat_lo and heat_issue_logged != "low":
                    tpdata = json.dumps(format_event(True))   # removed self.
                    try:
                        # Open connection for event update
                        ...
                        event_conn.commit()
                        machine_name = 'mc'+str(18)
                        message = """{"suggestion": {"HMI_Hor_Sealer_Strk_1": 55, "HMI_Hor_Sealer_Strk_2": 3, "text": "Increase Stroke 1 to 55", "HMI_Hor_Seal_Rear_28": 155, "front_temp": 155}}"""
                        #fault_message = self.fault_manager.get_message(machine_name, int(fault))
                        print(message)
                        query = """INSERT INTO suggestions("timestamp", machine_number, alert_details, suggestion_details, acknowledge) VALUES (%s, %s, %s, %s, 1);"""
                        params = (datetime.datetime.now(),'mc18','{"tp":61}',str(message))
                        event_cursor.execute(query,(params))
                        event_conn.commit()
                    except Exception as e:
                        log_event(f"Event DB update failed: {e}", latest_machine_ts)
                    finally:
