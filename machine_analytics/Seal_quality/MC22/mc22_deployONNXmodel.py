...
class promis_mc22:
    def __init__(self):
        ...
        # print("Loading model and scaler...")print("Loading model and scaler...")
        with open('/home/ai4m/develop/backup/develop/DQ/Seal_quality/MC22/MC22_seal_quality_deployment/mc22_seal_strength_scaler3.pkl', 'rb') as f:
    ...
    def predict_class(self,averaged_data):
        """
        Args : Sending pandas dataframe('Series') single row as input. 

        Description : 
        1. First,  we scale the data using the minmax scaler which is loaded as pickle file. 
        
        2. Second, we take that series data for seal_mc22 model input  (first converting to FP32 for hosted model) 
        and  pass the hosted model. 

        3. After inference , model returns the tensors which need to convert to numpy  array type. 

        4. Final, the results are returned.  numpy  [ '1d array' ]  
        ...
        scaled_input = np.array(scaled_input, dtype=np.float32).reshape(1, 7)  # Reshape it to match model input
        ...
        response = self.client.infer(model_name="Seal_mc22", inputs=[inputs], outputs=[outputs])
    ...
    def update_seal_strength(self,spare1, prediction):
        ...
        update_query = """UPDATE mc22 SET spare2 = %s WHERE spare1 = %s AND timestamp IN (SELECT timestamp FROM mc22 ORDER BY timestamp DESC LIMIT 100);"""
    ...
    def get_latest_spare1(self):
        ...
        cursor = conn.cursor()
        query = "SELECT spare1 FROM mc22_short_data ORDER BY timestamp DESC LIMIT 1;"
        cursor.execute(query)
...
if __name__ == "__main__":
    model = promis_mc22() ## Initializing the models. // 
    model.main() ## Excuting Model process // 
