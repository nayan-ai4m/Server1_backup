...
def read_plc_tag(tag_queue, plc_ip):
    while True:
        try:
            ...
            print(f"Failed to connect to PLC at {plc_ip}: {e}. Retrying in 5 seconds.")
            time.sleep(5)
            ...
            while True:
                try:
                    if not tag_queue.empty():
                        data = tag_queue.get()
                        timestamp = datetime.datetime(data['Year'], data['Month'], data['Day'], 
                    ...
                    print(f"Data dump for debugging: {data}")  # Added for debugging
                    time.sleep(1)
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            print(f"Error in main process: {e}. Restarting processes.")
            time.sleep(5)
