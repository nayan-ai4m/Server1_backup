...
def collect_data():  # Get recent 300 rows from table and return as dataframe
    try:
        ...
        cursor = conn.cursor()
        query = "SELECT * FROM mc30_fast ORDER BY timestamp DESC LIMIT 300;"
        ...
        print(f"Collected {len(df)} rows from mc30_fast, unique cycle_id values: {df['cycle_id'].unique()}")
        return df
...
def fetch_data_by_cycle_id(cycle_id):  # Get data for specific cycle_id from dataframe
    try:
        df = collect_data()
        if df.empty:
            print(f"No data fetched from mc30_fast for cycle_id {cycle_id}")
            return pd.DataFrame()
...
def insert_into_sync_data(cycle_id, vs_hs_sync, vs_hs_avg, vs_rv_sync, vs_rv_avg,
    ...
    try:
        ...
        insert_query = """
        INSERT INTO sync_data_30 (cycle_id, vertical_horizontal_sync, vertical_horizontal_sync_avg,
        ...
        conn.commit()
        print(f"Inserted sync data for cycle_id {cycle_id} into sync_data_30 at {timestamp}")
        cursor.close()
        conn.close()
    except Exception as e:
