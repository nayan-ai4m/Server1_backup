...
Reads PLC data from PostgreSQL (table: mc26_fast)
...
def get_latest_cycle_id():
    """Fetch the latest cycle_id from mc26_fast table."""
    with psycopg2.connect(**DB_CONFIG) as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT cycle_id FROM mc26_fast ORDER BY timestamp DESC LIMIT 1;")
            result = cur.fetchone()
