...
def fetch_latest_data(limit=300):
    ...
    try:
        ...
        cursor = conn.cursor()
        query = f"""
        SELECT pulling_servo_current/10.0 as pulling_servo_current,
        cam_position,
        spare1,
        timestamp,
        status
        ...
        columns = ['pulling_servo_current', 'cam_position', 'spare1', 'timestamp', 'machine_status']
        df = pd.DataFrame(rows, columns=columns)
        df['timestamp'] = pd.to_datetime(df['timestamp'])
        return df.sort_values('timestamp')
    except Exception as e:
        print(f"Error fetching data: {e}")
        return pd.DataFrame(columns=['pulling_servo_current', 'cam_position', 'spare1', 'timestamp', 'machine_status'])
    finally:
        if 'cursor' in locals():
            cursor.close()
        if 'conn' in locals():
...
def calculate_average_and_roc(df):
    ...
    range_data = df[mask]
    current_avg = range_data['pulling_servo_current'].mean()
    one_sec_ago = df['timestamp'].max() - timedelta(seconds=1)
    past_data = range_data[range_data['timestamp'] < one_sec_ago]
    past_avg = past_data['pulling_servo_current'].mean() if not past_data.empty else current_avg
...
def calculate_baseline(df):
    ...
    mask = (df['cam_position'] >= 50) & (df['cam_position'] <= 250)
    range_data = df[mask]
    baseline = range_data.groupby('cam_position')['pulling_servo_current'].mean()
