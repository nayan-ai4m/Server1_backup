...
def test_db_connection():
    """Test the database connection and verify the jamming_data_30 table exists."""
    try:
        conn = psycopg2.connect(**conn_params)
        cursor = conn.cursor()

        # Check if jamming_data_30 table exists
        cursor.execute("""
        SELECT EXISTS (
            SELECT FROM information_schema.tables
            WHERE table_name = 'jamming_data_30'
        );
        """)

        table_exists = cursor.fetchone()[0]
        if not table_exists:
            print("WARNING: The 'jamming_data_30' table does not exist!")
            print("Creating the table...")

            # Create the table if it doesn't exist
            cursor.execute("""
            CREATE TABLE IF NOT EXISTS jamming_data_30 (
            ...
            print("The 'jamming_data_30' table exists.")
...
def fetch_latest_data(limit=300):
    ...
    try:
        ...
        query = f"""
        SELECT
            puller_real_current_value_l/10.0 as puller_real_current_value_l,
            cam_position,
            cycle_id,
            timestamp,
            machine_status
        FROM mc30_short_data
        ORDER BY timestamp DESC
        LIMIT {limit};
        """
        cursor.execute(query)
        rows = cursor.fetchall()
        columns = ['puller_real_current_value_l', 'cam_position', 'cycle_id','timestamp', 'machine_status']
        df = pd.DataFrame(rows, columns=columns)

        # add cycle_id in column 'cycle_id'
        ...
        df = df[['puller_real_current_value_l', 'cam_position', 'cycle_id', 'timestamp', 'machine_status']]

        return df.sort_values('timestamp')
    except Exception as e:
        print(f"Error fetching data: {e}")
        return pd.DataFrame(columns=['puller_real_current_value_l', 'cam_position', 'cycle_id', 'timestamp', 'machine_status'])
    finally:
        if 'cursor' in locals():
            cursor.close()
        if 'conn' in locals():
...
def calculate_average_and_roc(df):
    ...
    range_data = df[mask]
    current_avg = range_data['puller_real_current_value_l'].mean()
    one_sec_ago = df['timestamp'].max() - timedelta(seconds=1)
    past_data = range_data[range_data['timestamp'] < one_sec_ago]
    past_avg = past_data['puller_real_current_value_l'].mean() if not past_data.empty else current_avg
...
def calculate_baseline(df):
    ...
    mask = (df['cam_position'] >= 50) & (df['cam_position'] <= 250)
    range_data = df[mask]
    baseline = range_data.groupby('cam_position')['puller_real_current_value_l'].mean()
    ...
    while True:
        ...
        if current_machine_status == 1 and not waiting_for_restart:
            ...
            if not baseline_data.empty:
                last_600_cycles = baseline_data['cycle_id'].unique()[-600:]
                baseline_df = baseline_data[baseline_data['cycle_id'].isin(last_600_cycles)]
                baseline, baseline_points = calculate_baseline(baseline_df)

                # Calculate metrics
                last_5_cycles = display_data['cycle_id'].unique()[-7:]
