...
def test_db_connection():
    """Test the database connection and verify the jamming_data_27 table exists."""
    try:
        conn = psycopg2.connect(**conn_params)
        cursor = conn.cursor()

        # Check if jamming_data_27 table exists
        cursor.execute("""
        SELECT EXISTS (
            SELECT FROM information_schema.tables
            WHERE table_name = 'jamming_data_27'
        );
        """)

        table_exists = cursor.fetchone()[0]
        if not table_exists:
            print("WARNING: The 'jamming_data_27' table does not exist!")
            print("Creating the table...")

            # Create the table if it doesn't exist
            cursor.execute("""
            CREATE TABLE IF NOT EXISTS jamming_data_27 (
            ...
            print("The 'jamming_data_27' table exists.")
