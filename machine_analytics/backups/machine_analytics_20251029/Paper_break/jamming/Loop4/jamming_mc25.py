...
def test_db_connection():
    """Test the database connection and verify/create the jamming_data_25 table."""
    try:
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()

        # Check if jamming_data_25 table exists
        cursor.execute("""
        SELECT EXISTS (
            SELECT FROM information_schema.tables
            WHERE table_name = 'jamming_data_25'
        );
        """)

        table_exists = cursor.fetchone()[0]
        if not table_exists:
            print("WARNING: The 'jamming_data_25' table does not exist!")
            print("Creating the table...")

            # Create the table if it doesn't exist
            cursor.execute("""
            CREATE TABLE IF NOT EXISTS jamming_data_25 (
            ...
            print("The 'jamming_data_25' table exists.")
...
# ===============================================================================
def main():
    """Main entry point for MC25 monitoring."""
    ...
    print("\n" + "="*60)
    print("MC25/MC25 Monitoring System Started")
    print("="*60)
