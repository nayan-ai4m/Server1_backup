...
def fetch_latest_data(limit=300):
    ...
    try:
        ...
        query = f"""
        SELECT
            puller_motor_current_value/10.0 as puller_motor_current_value,
            cam_position,
            cycle_id,
            timestamp,
            machine_status
        ...
        columns = ['puller_motor_current_value', 'cam_position', 'cycle_id','timestamp', 'machine_status']
        ...
        df = df[['puller_motor_current_value', 'cam_position', 'cycle_id', 'timestamp', 'machine_status']]

        return df.sort_values('timestamp')
    except Exception as e:
        print(f"Error fetching data: {e}")
        return pd.DataFrame(columns=['puller_motor_current_value', 'cam_position', 'cycle_id', 'timestamp', 'machine_status'])
    finally:
        if 'cursor' in locals():
            cursor.close()
        if 'conn' in locals():
...
def calculate_average_and_roc(df):
    ...
    range_data = df[mask]
    current_avg = range_data['puller_motor_current_value'].mean()
    one_sec_ago = df['timestamp'].max() - timedelta(seconds=1)
    past_data = range_data[range_data['timestamp'] < one_sec_ago]
    past_avg = past_data['puller_motor_current_value'].mean() if not past_data.empty else current_avg
...
def calculate_baseline(df):
    ...
    mask = (df['cam_position'] >= 50) & (df['cam_position'] <= 250)
    range_data = df[mask]
    baseline = range_data.groupby('cam_position')['puller_motor_current_value'].mean()
