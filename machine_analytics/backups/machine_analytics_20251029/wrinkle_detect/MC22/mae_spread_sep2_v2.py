...
class ThresholdMonitor:
    ...
    def format_event(self,is_state):
        ...
        "filepath": "http://192.168.0.158:8015/mc22_detection_1742302886.png",
        "color_code": 1,
        "machine_part": "plc"
        }   
    def connect(self):
    ...
    def generate_event_id(self, machine: str, status: int, timestamp: datetime) -> str:
        ...
        return f"{machine.upper()}{time_str}{status}"
    ...
    def update_pressure_spread(self, spread_value):
        """Update the latest row with calculated pressure_spread."""
        try:
            query = """
                UPDATE mc22_autoencoder
                SET pressure_spread = %s
                WHERE timestamp = (
                    SELECT timestamp FROM mc22_autoencoder
                    ORDER BY timestamp DESC
                    LIMIT 1
                )
            """
            self.cursor.execute(query, (spread_value,))
            self.conn.commit()
            print(f"Updated mc22_autoencoder with spread = {spread_value:.6f}")
        except Exception as e:
            print(f"Error updating pressure_spread: {e}")
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.update_pressure_spread(spread)    # Update the database with the new spread value
            ...
            if (2 * std_dev > 0.049) and self.wrinkle_flag == False:
                ...
                query = """UPDATE mc22_tp_status SET tp63 = '{}' """.format(json.dumps(data))
                print(query)
