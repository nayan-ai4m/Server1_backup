...
class ThresholdMonitor:
    ...
    def update_pressure_spread(self, spread_value):
        ...
        try:
            query = """
                UPDATE mc19_autoencoder
                SET pressure_spread = %s
                WHERE timestamp = (
                    SELECT timestamp FROM mc19_autoencoder
                    ORDER BY timestamp DESC
                    LIMIT 1
                )
            """
            ...
            self.conn.commit()
            print(f"Updated mc19_autoencoder with spread = {spread_value:.6f}")
        except Exception as e:
    ...
    def run(self):
        ...
        while self.running:
            ...
            if (2 * std_dev > 0.049) and self.wrinkle_flag == False:
                ...
                query = """UPDATE mc19_tp_status SET tp63 = '{}' """.format(json.dumps(data))
                print(query)
