...
class ConfigurableMachineAnalyzer:
        ...
        try:
            ...
            cursor.execute("""
                SELECT EXISTS (
                    ...
                    WHERE table_name = 'machine_config'
                );
            """)
            ...
            if not table_exists:
                self.logger.warning("machine_config table not found. Creating table...")
            ...
            for row in rows:
                ...
                self.machine_configs[machine_id] = {
                    ...
                    'const_threshold_1': row[10],
                    'const_threshold_2': row[11],
                    'approach_buffer': row[12],
                    'angle_threshold_low': row[13],
                    'angle_threshold_high': row[14],
                    'detection_history_size': row[15],
                    'is_active': row[16]
                }
    ...
    def create_config_table(self):
        """Create machine_config table if it doesn't exist"""
        try:
            cursor = self.short_data_conn.cursor()
            ...
            self.logger.info("Created machine_config table")
    ...
    def insert_default_configs(self):
        ...
        try:
            ...
            # Default configurations for MC17-MC22
            default_configs = [
                ('mc17', 'Machine 17', 'mc17_short_data', 'mc17', 'mc17_tp_status', 'tp66', 'tp67', 120, 190, 0.3, -3.0, -4.5, 0.2, 27, 67, 16),
                ('mc18', 'Machine 18', 'mc18_short_data', 'mc18', 'mc18_tp_status', 'tp66', 'tp67', 120, 190, 0.3, -3.0, -4.5, 0.2, 27, 67, 16),
                ('mc19', 'Machine 19', 'mc19_short_data', 'mc19', 'mc19_tp_status', 'tp66', 'tp67', 120, 190, 0.3, -3.0, -4.5, 0.2, 27, 67, 16),
                ('mc20', 'Machine 20', 'mc20_short_data', 'mc20', 'mc20_tp_status', 'tp66', 'tp67', 120, 190, 0.3, -3.0, -4.5, 0.2, 27, 67, 16),
                ('mc21', 'Machine 21', 'mc21_short_data', 'mc21', 'mc21_tp_status', 'tp66', 'tp67', 120, 190, 0.3, -3.0, -4.5, 0.2, 19, 67, 16),
                ('mc22', 'Machine 22', 'mc22_short_data', 'mc22', 'mc22_tp_status', 'tp66', 'tp67', 120, 190, 0.3, -3.0, -4.5, 0.2, 19, 67, 16),
            ]
            ...
            for config in default_configs:
                cursor.execute("""
                    INSERT INTO machine_config (
                        ...
                        tp_status_table, tp_overweight, tp_underweight, cam_min, cam_max,
                        threshold_value, const_threshold_1, const_threshold_2, approach_buffer,
...
class MachineAnalyzer:
    ...
    def fetch_hopper_data(self):
        ...
        try:
            ...
            self.logger.debug(f"[{self.machine_id}] latest_timestamp: {latest_timestamp}, fifteen_minutes_before: {fifteen_minutes_before}")
            ...
            self.logger.debug(f"[{self.machine_id}] Fetched hopper data: {df.shape}")
        ...
        try:
            ...
            if len(self.detection_history) < history_size:
                ...
                if all(detection == "POSSIBLE UNDERWEIGHT" for detection in self.detection_history):
                    ...
                    self.logger.info(f"[{self.machine_id}] Consensus: UNDERWEIGHT (All {history_size} detections agree)")
                elif all(detection == "POSSIBLE OVERWEIGHT" for detection in self.detection_history):
                    consensus_result = "POSSIBLE OVERWEIGHT"
                    self.logger.info(f"[{self.machine_id}] Consensus: OVERWEIGHT (All {history_size} detections agree)")
                else:
                    consensus_result = "NORMAL WEIGHT"

            # Event logging only if consensus changed
                ...
                if len(smoothed_in_range) > 0:
                    ...
                    if not pd.isna(min_idx):
                        ...
                        if min_cam in baseline.index:
                            ...
                            self.logger.debug(f"[{self.machine_id}] Types - min_val: {type(min_val)}, baseline_val: {type(baseline_val)}, threshold: {type(threshold)}")
                            if min_val < baseline_val - threshold:
                                self.current_viscosity_status = "High"
                                self.logger.debug(f"[{self.machine_id}] High viscosity detected")
                            elif min_val > baseline_val + threshold:
                                self.current_viscosity_status = "Low"
                                self.logger.debug(f"[{self.machine_id}] Low viscosity detected")
                            else:
                                self.current_viscosity_status = "Normal"
    ...
    def update_analysis(self):
        ...
        try:
            ...
            # Perform analysis when machine is running
