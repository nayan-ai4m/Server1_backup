...
class CombinedAnalyzer:
    def __init__(self):
        try:
            ...
            self.short_data_conn = psycopg2.connect(**self.short_data_conn_params)
            self.hul_event_conn = psycopg2.connect(**self.hul_event_conn_params)
            self.hul_tp_conn = psycopg2.connect(**self.hul_tp_conn_params)
    ...
    def __del__(self):
        ...
        try:
            ...
            if hasattr(self, 'hul_event_conn') and self.hul_event_conn:
                self.hul_event_conn.close()
            if hasattr(self, 'hul_tp_conn') and self.hul_tp_conn:
                self.hul_tp_conn.close()
        except Exception as e:
    ...
    def fetch_latest_filling_stroke(self):
        try:
            cursor = self.hul_event_conn.cursor()
            query = """
            WITH filtered_data AS (
            SELECT
            timestamp,
            mc21->>'HMI_Filling_Stroke_Deg' as filling_stroke_deg
            FROM loop3_checkpoints
            WHERE
            timestamp >= NOW() - INTERVAL '7 days'
            AND mc21->>'HMI_Filling_Stroke_Deg' IS NOT NULL
            ), ranked_data AS (
            SELECT
            timestamp,
            filling_stroke_deg,
    ...
    def fetch_hopper_data(self):
        try:
            ...
            """
            df = pd.read_sql_query(query, self.short_data_conn, params=[fifteen_minutes_before], parse_dates=['bucket_timestamp'])
            cursor.close()
            if df.empty:
    ...
    def handle_underweight_detection(self):
        ...
        try:
            ...
            cursor_event = self.hul_event_conn.cursor()
            cursor_tp = self.hul_tp_conn.cursor()
            ...
            update_query = """UPDATE mc21_tp_status SET tp67 = '{}' """.format(tp_data)
            cursor_tp.execute(update_query)
            self.hul_tp_conn.commit()
            cursor_event.close()
            cursor_tp.close()
    ...
    def handle_overweight_detection(self):
        ...
        try:
            ...
            update_query = """UPDATE mc21_tp_status SET tp66 = '{}' """.format(tp_data)
            cursor_tp.execute(update_query)
            self.hul_tp_conn.commit()
            # Flag Management
    ...
    def analyze_weight_status(self):
        try:
            ...
            if avg_angle < 19 and viscosity == "High":
                individual_detection = "POSSIBLE OVERWEIGHT"
            elif 19 <= avg_angle <= 67 and viscosity == "High":
                individual_detection = "POSSIBLE OVERWEIGHT"
            elif avg_angle > 67 and viscosity == "High":
                individual_detection = "POSSIBLE OVERWEIGHT"
            elif avg_angle < 19 and viscosity == "Low":
                individual_detection = "POSSIBLE UNDERWEIGHT"
            elif 19 <= avg_angle <= 67 and viscosity == "Low":
                individual_detection = "POSSIBLE UNDERWEIGHT"
            elif avg_angle > 67 and viscosity == "Low":
                individual_detection = "POSSIBLE UNDERWEIGHT"
            elif avg_angle < 19 and viscosity == "Normal":
                individual_detection = "POSSIBLE OVERWEIGHT"
