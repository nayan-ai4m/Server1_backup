...
class CombinedAnalyzer:
    def __init__(self):
        try:
            ...
            self.underweight_flag = False
            self.overweight_flag =  False
    ...
    def analyze_weight_status(self):
        try:
            ...
            if weight_status == "NORMAL WEIGHT":
                print("normal")
                if self.underweight_flag == True:
                    self.underweight_flag = False
                if self.overweight_flag == True:
                    self.overweight_flag = False
            if weight_status == "POSSIBLE UNDERWEIGHT" and self.underweight_flag == False:
                print("possible underweight")
                self.underweight_flag = True
                try:
                    cursor = self.hul_conn.cursor()
                    tp_data = json.dumps({
                    ...
                    event_data = (datetime.now().strftime("%Y%m%d_%H%M%S"), str(uuid.uuid4()), "PLC", "mc20", '', "POSSIBLE UNDERWEIGHT", "quality")
                    ...
                    query = """UPDATE mc20_tp_status SET tp67 = '{}' """.format(tp_data)
            ...
            if weight_status == "POSSIBLE OVERWEIGHT" and self.overweight_flag == False:
                self.overweight_flag = True
                ...
                try:
                    ...
                    event_data = (datetime.now().strftime("%Y%m%d_%H%M%S"), str(uuid.uuid4()), "PLC", "mc20", '', "POSSIBLE_OVERWEIGHT", "quality")
                    query = """UPDATE mc20_tp_status SET tp66 = '{}' """.format(tp_data)
                    insert_query = """INSERT INTO event_table (timestamp, event_id, zone, camera_id, filename, event_type, alert_type) VALUES (%s, %s, %s, %s, %s, %s, %s)"""
                    cursor.execute(insert_query, event_data)
                    self.hul_conn.commit()
