...
def fetch_latest_data(limit=300):
    ...
    try:
        ...
        columns = ['puller_real_current_value_l', 'cam_position', 'timestamp', 'machine_status']
        df = pd.DataFrame(rows, columns=columns)

        # add cycle_id in column 'spare1'
        df["timestamp"] = pd.to_datetime(df["timestamp"])
        position_diff = df['cam_position'].diff()
        cycle_change = position_diff < -180
        df['spare1'] = cycle_change.cumsum()

        # reorder columns if needed
        df = df[['puller_real_current_value_l', 'cam_position', 'spare1', 'timestamp', 'machine_status']]

        return df.sort_values('timestamp')
    except Exception as e:
        print(f"Error fetching data: {e}")
        return pd.DataFrame(columns=['puller_real_current_value_l', 'cam_position', 'spare1', 'timestamp', 'machine_status'])
    finally:
        if 'cursor' in locals():
            cursor.close()
        if 'conn' in locals():
