...
# Time range
start_time = "2025-09-03 09:15:40+05:30"
end_time = "2025-09-03 10:44:10+05:30"
...
def fetch_combined_data():
    try:
        ...
        # SQL query - Including status and status_code from mc22, temperature columns from mc22_mid
        query = text("""
        SELECT 
            mc22.timestamp,
            mc22.cam_position,
...
def fetch_with_custom_handling():
    ...
    try:
        ...
        # Query with COALESCE and including status columns from mc22
        query = text("""
        SELECT 
            mc22.timestamp,
            mc22.cam_position,
...
if __name__ == "__main__":
    ...
    if result_df is not None:
        ...
        # Verify we have the expected 10 columns including status columns
        expected_columns = [
        ...
        print(f"Column count - Expected: 10, Actual: {len(result_df.columns)}")
        ...
        print(f"\nStatus data summary:")
        print(f"Unique status values: {result_df['status'].unique()}")
        print(f"Unique status codes: {result_df['status_code'].unique()}")
        ...
        csv_filename = f'mc22_with_temperatures_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        result_df.to_csv(csv_filename, index=False)
        print(f"Data saved to '{csv_filename}'")

# Direct SQL query for PostgreSQL - NOW INCLUDING STATUS COLUMNS
sql_query_only = f"""
-- Direct SQL query for PostgreSQL - Including status columns from mc22
SELECT 
    mc22.timestamp,
    mc22.cam_position,
    mc22.hor_sealer_current/10 AS hor_sealer_current,
