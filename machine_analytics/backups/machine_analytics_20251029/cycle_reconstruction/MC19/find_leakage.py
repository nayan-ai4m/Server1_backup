...
class ThresholdMonitor:
    ...
    def format_event(self,is_state):
        ...
        "filepath": "http://192.168.0.158:8015/mc19_detection_1942302886.png",
        "color_code": 1,
        "machine_part": "plc"
        }   
    ...
    def insert_event(self):
        ...
        try:
            ...
            event_id = self.generate_event_id("mc19",'leakage', datetime.datetime.now())
            query = """
    ...
    def check_tp01_status(self):
        ...
        try:
            ...
            query = """SELECT tp01 FROM mc19_tp_status;"""
    ...
    def query_and_analyze(self):
        ...
        try:
            ...
            FROM mc19_autoencoder
            ORDER BY timestamp DESC
            LIMIT 10;
            """
            current_query = """select timestamp,hor_sealer_front_1_temp as front_current_value,hor_sealer_rear_1_temp as rear_current_value from mc19 order by timestamp desc limit 1;"""
            setpoint_query = """ select timestamp,(mc19->'HMI_Ver_Seal_Rear_28'->>'SetValue')::float as front_value,(mc19->'HMI_Ver_Seal_Rear_27'->>'SetValue')::float as rear_setvalue ,(mc19->'HMI_Hor_Sealer_Strk_1')::float as stroke1 , (mc19->'HMI_Hor_Sealer_Strk_2')::float as stroke2 from loop3_checkpoints where mc19 != 'null' order by timestamp desc limit 1;"""
            ...
            if total_rows == 0:
                print("No data found in mc19_autoencoder table")
                return
            ...
            for col_name, result in results.items():
                ...
                if result['exceeds_60_percent'] and self.flag_status[result['idx']] == False and self.check_tp01_status():
                    if col_name == 'mae_seal_pressure':
                        query = """select AVG(hor_pressure) from (select hor_pressure from mc19_short_data order by timestamp desc limit 100) as latest_values;"""
                        self.cursor.execute(query)
                        ...
                        if col_name == 'mae_seal_pressure':
                            if result['vector']:
                                ...
                                query = """UPDATE mc19_tp_status SET tp62 = %s"""
                                ...
                                data = {"machine":19,"type":"pressure","setpoint":setpoint_front_temp,"current":front_temp,"timestamp":str(datetime.datetime.now())}
                                self.producer.send(self.topic,value=data)
                                data = {"machine":19,"type":"pressure","setpoints":{'s1':54.0,'s2':3,'front':154,'rear':154},"current":{'front':154,'rear':154,'pressure':1.83},"timestamp":str(datetime.datetime.now()),"alert":tpdata}
                                self.producer.send(self.topic,value=data)
                                print(data)
                    ...
                    if col_name == 'mae_rear_temp' or col_name == 'mae_front_temp':
                        ...
                        if col_name == 'mae_front_temp':
                            if setpoint_front_temp > front_temp:
                                ...
                                tpdata = json.dumps(self.format_event(True))
                                query = """UPDATE mc19_tp_status SET tp61 = %s"""
                                self.event_cursor.execute(query, (tpdata,))
                                self.event_conn.commit()
                                ...
                                data = {"machine":19,"type":"front_temp","setpoints":{'s1':stroke1,'s2':stroke2,'front':setpoint_front_temp,'rear':setpoint_rear_temp},"current":{'front':front_temp,'rear':rear_temp},"timestamp":str(datetime.datetime.now()),"alert":tpdata}
                                self.producer.send(self.topic,value=data)
                            else:
                        ...
                        if col_name == 'mae_rear_temp':
                            ...
                            if setpoint_rear_temp == rear_temp:
                                ...
                                data = {"machine":19,"type":"rear_temp","setpoints":{'s1':stroke1,'s2':stroke2,'front':setpoint_front_temp,'rear':setpoint_rear_temp},"current":{'front':front_temp,'rear':rear_temp},"timestamp":str(datetime.datetime.now()),"alert":tpdata}
                                self.producer.send(self.topic,value=data)
                            else:
                    ...
                    if result['exceeds_60_percent'] == False and self.check_tp01_status():
                        if self.flag_status[result['idx']] == True:
                            ...
                            query = """UPDATE mc19_tp_status SET tp61 = 'null'"""
                            self.event_cursor.execute(query)
                            self.event_conn.commit()
...
if __name__ == "__main__":
    ...
    monitor = ThresholdMonitor(db_params, table_name='mc19_short_data', query_interval=3)
    monitor.run()
