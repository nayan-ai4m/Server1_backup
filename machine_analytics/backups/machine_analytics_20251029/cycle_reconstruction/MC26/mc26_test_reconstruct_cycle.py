...
def get_latest_spare1():
    ...
    cursor = conn.cursor()
    query = "SELECT spare1 FROM mc26_short_data ORDER BY timestamp DESC LIMIT 1;"
...
def fetch_data_by_spare1(spare1):
    ...
    short_columns = ['timestamp', 'cam_position', 'horizantal_motor_tq', 
                     'horizontal_sealer_position', 'spare1']
    df_short = pd.DataFrame(rows1, columns=short_columns)

    if df_short.empty:
        print(f"No short_data found for spare1: {spare1}")
        return pd.DataFrame()
    ...
    query2 = """
        SELECT hor_temp_28_pv_front, hor_temp_28_pv_rear
        FROM mc26_mid
    ...
    df_short['hor_temp_28_pv_front'] = front_temp
    df_short['hor_temp_28_pv_rear'] = rear_temp

    print(f"Data fetched for spare1: {spare1}, rows: {len(df_short)} (with latest temps from mc26_mid)")
    return df_short
...
def create_dict_data(df):                        # Pass df collected from query
    ...
    train_data = pd.DataFrame(columns=['cycle_id', 'horizantal_motor_tq', 'horizontal_sealer_position', 
                                     'hor_temp_28_pv_front', 'hor_temp_28_pv_rear'])
...
def scaled_2D_df(norm):               # Input is dataframe containing interpolated lists of len 37.
    ...
    norm['hor_temp_28_pv_front'] = norm['hor_temp_28_pv_front'].apply(lambda x: [(i - 120) / 50 for i in x])
...
def log_MAE_to_DB(row, table_name, conn_params, vectors):
    ...
    try:
        ...
        (cycle_id, timestamp, mae_motor_tq, mae_sealer_position, mae_front_temp, mae_rear_temp, motor_tq_vector, front_temp_vector, rear_temp_vector)
