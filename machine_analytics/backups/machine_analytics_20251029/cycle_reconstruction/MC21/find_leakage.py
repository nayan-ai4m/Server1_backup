...
class ThresholdMonitor:
    ...
    def format_event(self, is_state):
        return {
            ...
            "filepath": "http://192.168.0.158:8015/mc21_detection_2142302886.png",
            "color_code": 1,
            "machine_part": "plc"
        }
    ...
    def reconnect_if_needed(self):
        """Check and reconnect cursors if they're closed."""
        try:
            # Test if cursors are alive
            if self.cursor and self.cursor.closed:
                self.cursor = self.conn.cursor()
                print("Reconnected main cursor")
            if self.event_cursor and self.event_cursor.closed:
                self.event_cursor = self.event_conn.cursor()
                print("Reconnected event cursor")
            if self.raw_cursor and self.raw_cursor.closed:
                self.raw_cursor = self.raw_conn.cursor()
                print("Reconnected raw cursor")
                
            # Test if connections are alive
            if self.conn and self.conn.closed:
                self.conn = psycopg2.connect(**self.db_params)
                self.cursor = self.conn.cursor()
                print("Reconnected main connection")
            if self.event_conn and self.event_conn.closed:
                self.event_conn = psycopg2.connect(**self.event_db_params)
                self.event_cursor = self.event_conn.cursor()
                print("Reconnected event connection")
            if self.raw_conn and self.raw_conn.closed:
                self.raw_conn = psycopg2.connect(**self.raw_db_params)
                self.raw_cursor = self.raw_conn.cursor()
                print("Reconnected raw connection")
        except Exception as e:
    ...
    def generate_event_id(self, machine: str, status: str, timestamp: datetime) -> str:
    ...
    def insert_event(self):
        ...
        try:
            ...
            event_id = self.generate_event_id("mc21", 'leakage', datetime.datetime.now())
            query = """
    ...
    def check_tp01_status(self):
        ...
        try:
            ...
            query = """SELECT tp01 FROM mc21_tp_status;"""
    ...
    def query_and_analyze(self):
        ...
        try:
            ...
            FROM mc21_autoencoder
            ORDER BY timestamp DESC
            LIMIT 10;
            """
            current_query = """select timestamp,hor_sealer_front_1_temp as front_current_value,hor_sealer_rear_1_temp as rear_current_value from mc21 order by timestamp desc limit 1;"""
            setpoint_query = """ select timestamp,(mc21->'HMI_Ver_Seal_Rear_28'->>'SetValue')::float as front_value,(mc21->'HMI_Ver_Seal_Rear_27'->>'SetValue')::float as rear_setvalue ,(mc21->'HMI_Hor_Sealer_Strk_1')::float as stroke1 , (mc21->'HMI_Hor_Sealer_Strk_2')::float as stroke2 from loop3_checkpoints where mc21 != 'null' order by timestamp desc limit 1;"""
            ...
            if total_rows == 0:
                print("No data found in mc21_autoencoder table")
                return
            ...
            for col_name, result in results.items():
                ...
                print(f"leakage flag: {self.flag_status[result['idx']]}")
                ...
                if result['exceeds_60_percent'] and self.flag_status[result['idx']] == False and self.check_tp01_status():
                    if col_name == 'mae_seal_pressure':
                        query = """select AVG(hor_pressure) from (select hor_pressure from mc21_short_data order by timestamp desc limit 100) as latest_values;"""
                        self.cursor.execute(query)
                        rows = self.cursor.fetchone()
                        avg_pressure = rows[0] if rows else 0.0
                        ...
                        if col_name == 'mae_seal_pressure':
                            if result['vector']:
                                ...
                                query = """UPDATE mc21_tp_status SET tp62 = %s"""
                                self.event_cursor.execute(query, (tpdata,))
                                self.event_conn.commit()
                                
                                if self.check_tp01_status():  # Additional check before insert
                                    self.insert_event()
                                
                                data = {"machine": 21, "type": "pressure", "setpoint": setpoint_front_temp, "current": front_temp, "timestamp": str(datetime.datetime.now())}
                                self.producer.send(self.topic, value=data)
                                data = {"machine": 21, "type": "pressure", "setpoints": {'s1': 54.0, 's2': 3, 'front': 154, 'rear': 154}, "current": {'front': 154, 'rear': 154, 'pressure': 1.83}, "timestamp": str(datetime.datetime.now()), "alert": tpdata}
                                self.producer.send(self.topic, value=data)
                                print(data)
                    ...
                    if col_name == 'mae_rear_temp' or col_name == 'mae_front_temp':
                        ...
                        if col_name == 'mae_front_temp':
                            if setpoint_front_temp > front_temp:
                                print("Front temp decreased")
                                tpdata = json.dumps(self.format_event(True))
                                query = """UPDATE mc21_tp_status SET tp61 = %s"""
                                self.event_cursor.execute(query, (tpdata,))
                                self.event_conn.commit()
                                ...
                                data = {"machine": 21, "type": "front_temp", "setpoints": {'s1': stroke1, 's2': stroke2, 'front': setpoint_front_temp, 'rear': setpoint_rear_temp}, "current": {'front': front_temp, 'rear': rear_temp}, "timestamp": str(datetime.datetime.now()), "alert": tpdata}
                                self.producer.send(self.topic, value=data)
                            else:
                                print("Front temp increased")
                                
                        if col_name == 'mae_rear_temp':
                            print(f"Setpoint rear: {setpoint_rear_temp}, Current rear: {rear_temp}")
                            if setpoint_rear_temp == rear_temp:
                                print("Rear temp decreased")
                                tpdata = json.dumps(self.format_event(True))
                                query = """UPDATE mc21_tp_status SET tp61 = %s"""
                                self.event_cursor.execute(query, (tpdata,))
                                self.event_conn.commit()
                                ...
                                data = {"machine": 21, "type": "rear_temp", "setpoints": {'s1': stroke1, 's2': stroke2, 'front': setpoint_front_temp, 'rear': setpoint_rear_temp}, "current": {'front': front_temp, 'rear': rear_temp}, "timestamp": str(datetime.datetime.now()), "alert": tpdata}
                                self.producer.send(self.topic, value=data)
                            else:
                                print("Rear temp increased")
                else:
                    if result['exceeds_60_percent'] == False and self.check_tp01_status():
                        if self.flag_status[result['idx']] == True:
                            ...
                            query = """UPDATE mc21_tp_status SET tp61 = 'null'"""
                            self.event_cursor.execute(query)
                            self.event_conn.commit()
                            print(f"Reset flag for {col_name}")
...
if __name__ == "__main__":
    ...
    monitor = ThresholdMonitor(db_params, table_name='mc21_short_data', query_interval=3)
    monitor.run()
