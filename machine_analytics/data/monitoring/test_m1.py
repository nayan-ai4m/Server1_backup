...
CAMERA_RESPONSE_THRESHOLD_MS = 60  # Set threshold for camera response time in milliseconds

# List of IP addresses to check
IP_LIST = ["192.168.4.11"]
CAMERA_IP_LIST = ["192.168.1.21" ,"192.168.1.20","192.168.1.22","192.168.1.27","192.168.1.25","192.168.1.23","192.168.1.24","192.168.1.26","192.168.1.29","192.168.1.28","192.168.1.30"] # Example camera IPs
...
def collect_system_info(server1):
    ...
    system_info['cpu'] = {
        'total_usage_percentage': total_cpu_percentage
    }
...
class ServerMonitor:
    ...
    def collect_stats(self, server1):
        ...
        try:
            ...
            gpu_data = get_gpu_stats()
            if isinstance(gpu_data, list) and len(gpu_data) > 0:
                gpu_usage = gpu_data[0]["GPU Usage (%)"]
