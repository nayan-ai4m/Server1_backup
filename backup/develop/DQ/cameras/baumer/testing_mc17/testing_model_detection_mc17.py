...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/testing_mc17/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
    ...
    # Fix the validation to handle NumPy arrays properly
        ...
        for box, cls in zip(boxes, classes):
            ...
            x1, y1, x2, y2 = box
            name, color = self.models['slitting_model'].classes.get(int(cls), ("custom", (0, 0, 255)))
            cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)
            cv2.putText(img, name, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        return img
...
if __name__ == '__main__':
    config = {
        ...
        'models': {
            'slitting_model': {'url': '0.0.0.0:9002', 'model_name': 'slitting_model'}
        },
    ...
    try:
        while True:
            ...
            if socket_mc17 in socks and socks[socket_mc17] == zmq.POLLIN:
                ...
                #cv2.imwrite("/home/ai4m/develop/backup/develop/DQ/cameras/baumer/testing_mc17/mc17"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".png",frame)
