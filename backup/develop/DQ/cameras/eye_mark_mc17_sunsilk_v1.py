...
class DetectionProcessor:
    ...
    def process(self, img):
        ...
        while threading.active_count() > 1 or not self.q.empty():
            if not self.q.empty():
                ...
                cv2.imwrite("baumer/mc17/may23/inference/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",image)
    ...
    def _handle_detection(self, prefix, boxes, classes, filename, tp_column, img):
        ...
        event_data = (now, str(uuid.uuid4()), "Baumer Camera", "MC17", filename, "defect", "quality")
        tp_data = json.dumps({
    ...
    try:
        while True:
            ...
            if socket_mc17 in socks and socks[socket_mc17] == zmq.POLLIN:
                ...
                cv2.imwrite("baumer/mc17/may23/data_may23/"+str(datetime.datetime.now().strftime("%m_%d_%Y_%H_%M_%S_%p"))+".jpeg",frame)
