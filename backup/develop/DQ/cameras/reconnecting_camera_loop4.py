...
if __name__ == "__main__":
    ...
    cam_stream = Reconnecting_CamGear(
        cam_address="rtsp://admin:unilever2024@192.168.1.28:554/Streaming/Channels/103",
        reset_attempts=2000,
        reset_delay=5,
    )


    stream = Stream("loop4", size=(1280, 720), quality=50, fps=10)

    server = MjpegServer("192.168.0.158", 3003)  #192.168.4.11
    server1 = MjpegServer("192.168.0.158", 3006)  #192.168.4.11
    server.add_stream(stream)
    server1.add_stream(stream)
    server.start()
    server1.start()
