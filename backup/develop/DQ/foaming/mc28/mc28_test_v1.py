...
MC28_FOAM_DATABASE_URL = 'postgresql://postgres:ai4m2024@localhost/short_data_hul'
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("mc28_machine_monitor.log"),
...
recent_thresholds = {}
cache_file_path = "mc28_threshold_cache.json"
...
def check_machine_status():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT status, NOW() FROM mc28 ORDER BY timestamp DESC LIMIT 1;")
...
def log_machine_stopped(timestamp):
    """
    Log an entry in the mc28_foam table when the machine is stopped.
    """
    try:
        conn = psycopg2.connect(MC28_FOAM_DATABASE_URL)
        cursor = conn.cursor()

        insert_query = """
        INSERT INTO mc28_foam (result_timestamp, cam_cycle_group, status, difference)
        VALUES (%s, NULL, 'Machine Stopped', NULL);
        """
        cursor.execute(insert_query, (timestamp,))
        conn.commit()
        logging.info(f"Machine stopped logged in mc28_foam at {timestamp}.")
...
def calculate_average_threshold():
    ...
    try:
        conn = psycopg2.connect(DATABASE_URL)
        cursor = conn.cursor()
        cursor.execute("SELECT MAX(timestamp) FROM mc28;")
...
def save_results_to_foam_db(results):
    try:
        ...
        conn.commit()
        logging.info("Results saved to mc28_foam table successfully.")
