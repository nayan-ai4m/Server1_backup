...
int main() {
    ...
    int returncode = 0;
    char * cpTargetBuffer = NULL;
    bool bPerformUpdate = false;
    int iTargetIndex = -1;

    std::cout << std::endl;
    std::cout << "#########################################################" << std::endl;
    std::cout << "# PROGRAMMER'S GUIDE Example 202_ShadingTableUpdate.cpp #" << std::endl;
    std::cout << "#########################################################" << std::endl;
    ...
    try {
        ...
        if (pDevice->IsUpdateModeActive()) {
            ...
            bo_int64 iTargetNumber = 0;
            if (pDevice->GetUpdateNode("TargetSelector")->GetAvailable()) {
                iTargetNumber = pDevice->GetUpdateNode("TargetSelector")->GetIntMax() + 1;
            }
            std::cout << "    Number of available Targets:           " << iTargetNumber << std::endl;
            for (int i = 0; i < iTargetNumber; i++) {
                pDevice->GetUpdateNode("TargetSelector")->SetInt(i);
                std::cout << "        Targetname:                       " << std::setw(2) << i << ":    "
                    << pDevice->GetUpdateNode("TargetName")->GetValue() << std::endl;
            }
            bPerformUpdate = iTargetNumber > 0;
        }
    }
    catch (BGAPI2::Exceptions::IException& ex) {
        returncode = (returncode == 0) ? 1 : returncode;
    ...
    if (bPerformUpdate) {
        ...
        std::cout << "##############" << std::endl << std::endl;
        std::cout << "    Input a valid index to start the update of a target or any other number to skip: ";
        std::string sTargetIndex;
        std::getline(std::cin, sTargetIndex);
        std::stringstream sInput(sTargetIndex);
        sInput >> iTargetIndex;
        try {
            bPerformUpdate = iTargetIndex <= pDevice->GetUpdateNode("TargetSelector")->GetIntMax();
            bPerformUpdate &= iTargetIndex >= pDevice->GetUpdateNode("TargetSelector")->GetIntMin();
            if (bPerformUpdate) {
                // Load Target
                std::cout << std::endl << "    Enter a valid file name (including path): ";
                std::string sUpdatePath;
                std::getline(std::cin, sUpdatePath);
                std::ifstream streamInput(sUpdatePath.c_str(), std::fstream::binary);
                if (streamInput.is_open()) {
                    streamInput.seekg(0, streamInput.end);
                    size_t uiLength = streamInput.tellg();
                    streamInput.seekg(0, streamInput.beg);
                    cpTargetBuffer = new char[uiLength];
                    streamInput.read(cpTargetBuffer, uiLength);
                    streamInput.close();
                    pDevice->GetUpdateNode("UpdateData")->set(cpTargetBuffer, uiLength);
                } else {
                    bPerformUpdate = false;
                    std::cout << "Open of '" << sUpdatePath << "' was not possible - Update will be skipped!"
                        << std::endl;
                }
            }
        }
    ...
    } else if (pDevice->IsUpdateModeActive()) {
        std::cout << "No Targets available - Update will be skipped!" << std::endl;
    }
    ...
    if (bPerformUpdate) {
        try {
            // CHECK IF UPDATE MODE IS ACTIVE
            if (pDevice->IsUpdateModeActive()) {
                for (int i = 0; i <= pDevice->GetUpdateNode("TargetSelector")->GetIntMax(); i++) {
                    pDevice->GetUpdateNode("TargetSelector")->SetInt(i);
                    if (pDevice->GetUpdateNode("TargetName")->GetValue() == "ShadingTable") {
                        std::cout << "    Start update!" << std::endl;
                        pDevice->GetUpdateNode("StartTargetUpdate")->Execute();
                        while (pDevice->GetUpdateNode("DeviceState")->GetValue() == "Updating") {
                            std::cout << "    Update Progress:  " << std::setw(4)
                                << pDevice->GetUpdateNode("UpdateProgress")->GetValue() << "%" << '\r' << std::flush;
#if defined(_WIN32)
                            Sleep(1000);
#else
                            usleep(1000000);
        ...
        try {
            ...
            if (pDevice->IsUpdateModeActive()) {
                if (pDevice->GetUpdateNode("StartReboot")->GetAvailable()) {
                    ...
                    std::cout << "    Reboot is not supported by this device, please restart it manually."
                        << std::endl;
                }
            }
        }
