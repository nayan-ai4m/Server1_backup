...
int main() {
    ...
    std::cout << std::endl;
    std::cout << "###################################################" << std::endl;
    std::cout << "# PROGRAMMER'S GUIDE Example 201_DeviceUpdate.cpp #" << std::endl;
    std::cout << "###################################################" << std::endl;
    ...
    try {
        ...
        if (pDevice->IsUpdateModeActive()) {
            ...
            std::string sDeviceConfigID = pDevice->GetUpdateNode("DeviceConfigID")->GetValue().get();
            ...
            bo_int64 iUpdateNumber = 0;
            if (pDevice->GetUpdateNode("ConfigIDSelector")->GetAvailable()) {
                iUpdateNumber = pDevice->GetUpdateNode("ConfigIDSelector")->GetIntMax() + 1;
            }
            std::cout << "    Number of available Updates:           " << iUpdateNumber << std::endl;
            for (int i = 0; i < iUpdateNumber; i++) {
                pDevice->GetUpdateNode("ConfigIDSelector")->SetInt(i);
                std::cout << "        Updateversion:             [index "
                    << std::setw(2) << i << "]:   " << pDevice->GetUpdateNode("ConfigIDName")->GetValue();
                if (sDeviceConfigID > std::string(pDevice->GetUpdateNode("ConfigIDName")->GetValue().get())) {
                    std::cout << " (old)";
                } else if (sDeviceConfigID == std::string(pDevice->GetUpdateNode("ConfigIDName")->GetValue().get())) {
                    std::cout << " (current)";
                } else if (sDeviceConfigID < std::string(pDevice->GetUpdateNode("ConfigIDName")->GetValue().get())) {
                    std::cout << " (new)";
                }
            ...
            bPerformUpdate = iUpdateNumber > 0;
        }
    }
    catch (BGAPI2::Exceptions::IException& ex) {
        returncode = (returncode == 0) ? 1 : returncode;
    ...
    if (bPerformUpdate) {
        ...
        std::string sUpdateIndex;
        std::getline(std::cin, sUpdateIndex);
        std::stringstream sInput(sUpdateIndex);
        sInput >> iUpdateIndex;
        std::cout << "  " << std::endl;
        try {
            bPerformUpdate = iUpdateIndex <= pDevice->GetUpdateNode("ConfigIDSelector")->GetIntMax();
            bPerformUpdate &= iUpdateIndex >= pDevice->GetUpdateNode("ConfigIDSelector")->GetIntMin();
        }
        catch (BGAPI2::Exceptions::IException& ex) {
            returncode = (returncode == 0) ? 1 : returncode;
            std::cout << "ExceptionType:    " << ex.GetType() << std::endl;
    ...
    if (bPerformUpdate) {
        try {
            ...
            if (pDevice->IsUpdateModeActive()) {
                if (pDevice->GetUpdateNode("ConfigIDSelector")->GetAvailable()) {
                    pDevice->GetUpdateNode("ConfigIDSelector")->SetInt(iUpdateIndex);
                    std::cout << "    Selected Device Config ID (Firmware Version):   "
                        << pDevice->GetUpdateNode("ConfigIDName")->GetValue() << std::endl;
                }
                std::cout << "    Start update!" << std::endl;
                pDevice->GetUpdateNode("StartUpdate")->Execute();
