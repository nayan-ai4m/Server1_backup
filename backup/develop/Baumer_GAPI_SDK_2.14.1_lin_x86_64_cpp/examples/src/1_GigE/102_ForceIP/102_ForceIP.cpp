    ...
    try {
        ...
        for (BGAPI2::DeviceList::iterator devIterator = deviceList->begin();
            ...
            if (devIterator->GetTLType() == "GEV") {
                ...
                BGAPI2::NodeMap * pDeviceNodeMap = devIterator->GetNodeList();

                std::cout << "          Device MAC address (hex):      "
                    << std::hex << std::setfill('0') << std::setw(2)
                    << ((pDeviceNodeMap->GetNode("GevDeviceMACAddress")->GetInt() & 0xffffffffffffLL) >> 40)
                    << ":" << std::setw(2)
                    << ((pDeviceNodeMap->GetNode("GevDeviceMACAddress")->GetInt() & 0xffffffffffLL) >> 32)
                    << ":" << std::setw(2)
                    << ((pDeviceNodeMap->GetNode("GevDeviceMACAddress")->GetInt() & 0xffffffff) >> 24)
                    << ":" << std::setw(2)
                    << ((pDeviceNodeMap->GetNode("GevDeviceMACAddress")->GetInt() & 0xffffff) >> 16)
                    << ":" << std::setw(2)
                    << ((pDeviceNodeMap->GetNode("GevDeviceMACAddress")->GetInt() & 0xffff) >> 8)
                    << ":" << std::setw(2)
                    << (pDeviceNodeMap->GetNode("GevDeviceMACAddress")->GetInt() & 0xff)
                    << std::setfill(' ') << std::dec << std::endl;
                // DEVICE IP ADDRESS
                iDeviceIPAddress = pDeviceNodeMap->GetNode("GevDeviceIPAddress")->GetInt();
                std::cout << "          Device IP address (hex):       0x"
                    << std::nouppercase << std::setfill('0') << std::hex << std::setw(8) << iDeviceIPAddress
                    << std::nouppercase << std::setfill(' ') << std::dec << std::endl;
                std::cout << "          Device IP address (string):    "
                    << pDeviceNodeMap->GetNode("GevDeviceIPAddress")->GetValue() << std::endl;
                // DEVICE SUBNET MASK
                iDeviceSubnetMask = pDeviceNodeMap->GetNode("GevDeviceSubnetMask")->GetInt();
                std::cout << "          Device Subnet mask (hex):      0x"
                    << std::nouppercase << std::setfill('0') << std::hex << std::setw(8) << iDeviceSubnetMask
                    << std::nouppercase << std::setfill(' ') << std::dec << std::endl;
                std::cout << "          Device Subnet mask (string):   "
                    << pDeviceNodeMap->GetNode("GevDeviceSubnetMask")->GetValue() << std::endl;
                ...
                std::cout << "          Device Subnet (hex):           0x"
                    << std::nouppercase << std::setfill('0') << std::hex << std::setw(8) << iDeviceSubnet
                    << std::nouppercase << std::setfill(' ') << std::dec << std::endl;
                std::cout << "          Device Subnet (string):        "
                    << ((iDeviceSubnet & 0xffffffff) >> 24) << "."
                    << ((iDeviceSubnet & 0xffffff) >> 16) << "."
                    << ((iDeviceSubnet & 0xffff) >> 8) << "."
                    << (iDeviceSubnet & 0xff) << std::endl;

                iInterfaceSubnet = ((unsigned int)pInterface->GetNode("GevInterfaceSubnetIPAddress")->GetInt()) &
                    ((unsigned int)pInterface->GetNode("GevInterfaceSubnetMask")->GetInt());  // bit wise AND
                std::cout << "          Interface Subnet (hex):        0x"
                    << std::nouppercase << std::setfill('0') << std::hex << std::setw(8) << iInterfaceSubnet
                    << std::nouppercase << std::setfill(' ') << std::dec << std::endl;
                std::cout << "          Interface Subnet (string):     "
                    << ((iInterfaceSubnet & 0xffffffff) >> 24) << "."
                ...
                if (iDeviceSubnet == iInterfaceSubnet) {
                    ...
                    std::cout << "          IP address of device does not match to IP address of interface "
                        << std::endl << std::endl;
                    ...
                    try {
                        ...
                        std::cout << "          Forced IP Address (hex):       0x"
                            << std::nouppercase << std::setfill('0') << std::hex << std::setw(8)
                            << iForceIPAddress << std::nouppercase << std::setfill(' ') << std::dec << std::endl;
                        std::cout << "          Forced IP Address (string):    "
                            << ((iForceIPAddress & 0xffffffff) >> 24) << "."
                            << ((iForceIPAddress & 0xffffff) >> 16) << "."
                            << ((iForceIPAddress & 0xffff) >> 8) << "."
                            << (iForceIPAddress & 0xff) << std::endl;
                        std::cout << "          Interface IP Address (hex):    0x"
                            << std::nouppercase << std::setfill('0') << std::hex << std::setw(8)
                            << pInterface->GetNode("GevInterfaceSubnetIPAddress")->GetInt() << std::nouppercase
                            << std::setfill(' ') << std::dec << std::endl;
                        std::cout << "          Interface IP Address (string):    "
                            << pInterface->GetNode("GevInterfaceSubnetIPAddress")->GetValue() << std::endl;
                        ...
                        if (iForceIPAddress == pInterface->GetNode("GevInterfaceSubnetIPAddress")->GetInt()) {
                            ...
                            std::cout << "          Warning. Created IP address is already used by the interface "
                                << std::endl;
                            std::cout << "          Try next Forced IP Address " << std::endl;
                            std::cout << "          Forced IP Address (hex):       0x"
                                << std::nouppercase << std::setfill('0') << std::hex << std::setw(8) << iForceIPAddress
                                << std::nouppercase << std::setfill(' ') << std::dec << std::endl;
                            std::cout << "          Forced IP Address (string):    "
                                << ((iForceIPAddress & 0xffffffff) >> 24) << "."
                                << ((iForceIPAddress & 0xffffff) >> 16) << "."
                        ...
                        std::cout << "          Forced Subnet Mask (hex):      0x"
                            << std::nouppercase << std::setfill('0') << std::hex << std::setw(8)
                            << iInterfaceSubnetMask << std::nouppercase << std::setfill(' ') << std::dec << std::endl;
                        std::cout << "          Forced Subnet Mask (string):     "
                        ...
                        std::cout << "          Forced Gateway (hex):          0x"
                            << std::nouppercase << std::setfill('0') << std::hex << std::setw(8) << iForceIPGateway
                            << std::nouppercase << std::setfill(' ') << std::dec << std::endl;
                        std::cout << "          Forced Gateway (string):       "
                            << ((iForceIPGateway & 0xffffffff) >> 24) << "."
