...
int main() {
    ...
    try {
        ...
        std::cout << "        TriggerSource:            "
            << pDevice->GetRemoteNode("TriggerSource")->GetValue() << std::endl;
        ...
        if (bIsAvailableEventSelector &&
            pDevice->GetRemoteNode("EventSelector")->GetEnumNodeList()->GetNodePresent("ExposureStart")) {
        ...
        if (bIsAvailableEventSelector &&
            pDevice->GetRemoteNode("EventSelector")->GetEnumNodeList()->GetNodePresent("ExposureEnd")) {
        ...
        if (bIsAvailableEventSelector &&
            pDevice->GetRemoteNode("EventSelector")->GetEnumNodeList()->GetNodePresent("FrameTransferEnd")) {
        ...
        if ((bIsAvaliableEventSelectorExposureStart == true) || (bIsAvaliableEventSelectorExposureEnd == true)) {
            ...
            if ((pDevice->GetRemoteNode("EventNotification")->GetEnumNodeList()->GetNodePresent("On")) == true) {
                ...
                // TXG, SXG
                pDevice->GetRemoteNode("EventNotification")->SetValue("GigEVisionEvent");
                std::cout << "        EventNotification:        "
                    << pDevice->GetRemoteNode("EventNotification")->GetValue() << std::endl;
            }
            std::cout << std::endl;
        ...
        bo_uint64 nodeCount = pDevice->GetRemoteNode("EventSelector")->GetEnumNodeList()->GetNodeCount();
        for (bo_uint64 i = 0; bIsAvailableEventSelector && i < nodeCount; i++) {
    ...
    try {
        ...
        for (BGAPI2::DataStreamList::iterator dstIterator = datastreamList->begin();
            ...
            dstIterator++) {
            std::cout << "  5.2.4   DataStream ID:          "
                << dstIterator->GetID() << std::endl << std::endl;
        }
    }
    catch (BGAPI2::Exceptions::IException& ex) {
        returncode = (returncode == 0) ? 1 : returncode;
            ...
            if ((bIsAvaliableEventSelectorExposureStart == true) || (bIsAvaliableEventSelectorExposureEnd == true)) {
                if (bIsAvaliableEventSelectorExposureStart) {
                    ...
                    if (pDevice->GetDeviceEvent(pdEvent, 1000) == true) {  // timeout 1000 msec
                        try {
                            fMessageTimestampMicroSeconds_ExposureStart =
                                (bo_double)pdEvent->GetTimeStamp() / fTimestampTickFrequency * 1000000.0;
                        } catch (BGAPI2::Exceptions::IException&) {
                            fMessageTimestampMicroSeconds_ExposureStart = 0;
                        }
                        ...
                        std::cout << " EventID " << sEventId.c_str() << " "
                            << std::setw(14) << sEventName.c_str() << " Timestamp "
                            << std::fixed << std::setprecision(1)
                            << fMessageTimestampMicroSeconds_ExposureStart << " [usec]" << std::endl;
                    } else {
                ...
                if (bIsAvaliableEventSelectorExposureEnd) {
                    ...
                    if (pDevice->GetDeviceEvent(pdEvent, 1000) == true) {  // timeout 1000 msec
                        try {
                            fMessageTimestampMicroSeconds_ExposureEnd =
                                (bo_double)pdEvent->GetTimeStamp() / fTimestampTickFrequency * 1000000.0;
                        } catch (BGAPI2::Exceptions::IException&) {
                            fMessageTimestampMicroSeconds_ExposureEnd = 0;
                        }
                        std::string sEventId = "", sEventName = "";
                        try { sEventId = pdEvent->GetId().get(); } catch (BGAPI2::Exceptions::IException&) { sEventId = ""; }
                        try { sEventName = pdEvent->GetName().get(); } catch (BGAPI2::Exceptions::IException&) { sEventName = ""; }
                        std::cout << " EventID " << sEventId.c_str() << " "
                            << std::setw(14) << sEventName.c_str() << " Timestamp "
                            << std::fixed << std::setprecision(1) << fMessageTimestampMicroSeconds_ExposureEnd << " [usec]"
                            << std::endl;
                    } else {
                        std::cout << "Error: GetDeviceEvent Timeout after 1000 msec" << std::endl;
                    }
                ...
                if ((bIsAvaliableEventSelectorExposureStart == true) && (bIsAvaliableEventSelectorExposureEnd == true)) {
                    ...
                    fMessageTimestampMicroSeconds_ExposureDiff =
                        fMessageTimestampMicroSeconds_ExposureEnd - fMessageTimestampMicroSeconds_ExposureStart;
                    std::cout << " Timestamp ExposureEnd - ExposureStart: "
                        << std::fixed << std::setprecision(1) << fMessageTimestampMicroSeconds_ExposureDiff << " [usec]"
                        << std::endl;
                }
            } else if (bIsAvaliableEventSelectorFrameTransferEnd == true) {
                ...
                if (pDevice->GetDeviceEvent(pdEvent, 1000) == true) {  // timeout 1000 msec
                    ...
                    try {
                        fMessageTimestampMicroSecond =
                            (bo_double)pdEvent->GetTimeStamp() / fTimestampTickFrequency * 1000000.0;
                    }
                    catch (BGAPI2::Exceptions::IException&) {
                        fMessageTimestampMicroSecond = 0;
                    }
                    std::string sEventId = "", sEventName = "";
                    ...
                    std::cout << " EventID " << sEventId.c_str() << " "
                        << std::setw(14) << sEventName.c_str() << " Timestamp "
                        << std::fixed << std::setprecision(1) << fMessageTimestampMicroSecond << " [usec]"
                        << std::endl;
                } else {
                    std::cout << "Error: GetDeviceEvent Timeout after 1000 msec" << std::endl;
                }
