...
int main() {
    ...
    std::cout << std::endl;
    std::cout << "##########################################################" << std::endl;
    std::cout << "# PROGRAMMER'S GUIDE Example 003_CameraConfiguration.cpp #" << std::endl;
    std::cout << "##########################################################" << std::endl;
    ...
    try {
        ...
        std::cout << "          description:            "
            << pDevice->GetRemoteNode(sExposureNodeName)->GetDescription() << std::endl;
        std::cout << "          interface type:         "
            << pDevice->GetRemoteNode(sExposureNodeName)->GetInterface() << std::endl;
        bo_double fExposureTime = 0;
        bo_double fExposureTimeMin = 0;
        bo_double fExposureTimeMax = 0;

        // get current value and limits
        fExposureTime = pDevice->GetRemoteNode(sExposureNodeName)->GetDouble();
        ...
        std::cout << "          current value:          "
            << std::fixed << std::setprecision(0) << fExposureTime << std::endl;
        std::cout << "          possible value range:   "
            << std::fixed << std::setprecision(0) << fExposureTimeMin << " to " << fExposureTimeMax << std::endl;
        ...
        pDevice->GetRemoteNode(sExposureNodeName)->SetDouble(exposurevalue);
        ...
        std::cout << "          current value:          "
            << iImageWidth << std::endl;
        std::cout << "          possible value range:   "
            << iImageWidthMin << " to " << iImageWidthMax << " with increment of " << iImageWidthInc << std::endl;

        // set new width value same to the value of height
        // find number to match the increment
        bo_int64 widthvalue = pDevice->GetRemoteNode("Height")->GetInt() / iImageWidthInc * iImageWidthInc;
        ...
        std::cout << "          set value to:           " << pDevice->GetRemoteNode("Width")->GetInt()
            << " is about same as the height: " << pDevice->GetRemoteNode("Height")->GetInt()
            << std::endl << std::endl;
        ...
        // TRIGGER SOURCE
        std::cout << "5.3.4   TriggerSource" << std::endl;
        std::cout << "          description:            "
            << pDevice->GetRemoteNode("TriggerSource")->GetDescription() << std::endl;
        std::cout << "          interface type:         "
            << pDevice->GetRemoteNode("TriggerSource")->GetInterface() << std::endl;
        std::cout << "          enumeration list count: "
            << pDevice->GetRemoteNode("TriggerSource")->GetEnumNodeList()->GetNodeCount()
            << " (current marked with *)" << std::endl;
        for (bo_uint64 i = 0; i < pDevice->GetRemoteNode("TriggerSource")->GetEnumNodeList()->GetNodeCount(); i++) {
            pNode = pDevice->GetRemoteNode("TriggerSource")->GetEnumNodeList()->GetNodeByIndex(i);
            if (pNode->IsReadable()) {
                std::cout << " " << std::setw(30) << i << ": ";
                if (pNode->GetValue() == pDevice->GetRemoteNode("TriggerSource")->GetString()) {
                    std::cout << "*";
                } else {
                    std::cout << " ";
                }
        ...
        std::cout << "          description:            "
            << pDevice->GetRemoteNode("TriggerMode")->GetDescription() << std::endl;
        std::cout << "          interface type:         "
            << pDevice->GetRemoteNode("TriggerMode")->GetInterface() << std::endl;
        std::cout << "          current value:          "
        ...
        bool IsAvailableLineInverter = false;
        for (bo_uint64 i = 0; i < pDevice->GetRemoteNodeList()->GetNodeCount(); i++) {
            if (pDevice->GetRemoteNodeList()->GetNodeByIndex(i)->GetName() == "LineInverter") {
                IsAvailableLineInverter = true;
                break;
            }
        }
        if (IsAvailableLineInverter == true) {
            // LineInverter

            // select an I/O line first
            // LineSelector is interface type IEnumeration, see example TriggerSource
            pDevice->GetRemoteNode("LineSelector")->SetString("Line0");

            std::cout << "5.3.5   LineInverter" << std::endl;
            std::cout << "          description:            "
                << pDevice->GetRemoteNode("LineInverter")->GetDescription() << std::endl;
            std::cout << "          interface type:         "
                << pDevice->GetRemoteNode("LineInverter")->GetInterface() << std::endl;
            std::cout << "          current value:          "
                << pDevice->GetRemoteNode("LineInverter")->GetBool() << std::endl;
            std::cout << "          Line0 LineStatus:       "
                << pDevice->GetRemoteNode("LineStatus")->GetBool() << std::endl << std::endl;
    ...
    try {
        ...
        std::cout << "5.3.6   DeviceUserID" << std::endl;
        std::cout << "          description:            "
            << pDevice->GetRemoteNode("DeviceUserID")->GetDescription() << std::endl;
        std::cout << "          interface type:         "
            << pDevice->GetRemoteNode("DeviceUserID")->GetInterface() << std::endl;
        std::cout << "          current value:          "
            << pDevice->GetRemoteNode("DeviceUserID")->GetString() << std::endl;

        // SET A NEW USER ID LIKE "Camera0"
        BGAPI2::String newDeviceUserID = "Camera0";
        BGAPI2::String oldDeviceUserID = pDevice->GetRemoteNode("DeviceUserID")->GetString();
        ...
        if (newDeviceUserID.size() <= pDevice->GetRemoteNode("DeviceUserID")->GetMaxStringLength()) {
            ...
            std::cout << "Error: newDeviceUserID string length ("
                << newDeviceUserID.size() << ") is longer than MaxStringthLength ("
                << pDevice->GetRemoteNode("DeviceUserID")->GetMaxStringLength() << ")" << std::endl;
        }
        ...
        // RESET USER ID
        pDevice->GetRemoteNode("DeviceUserID")->SetString(oldDeviceUserID);
        std::cout << "          reset value to:         "
            << pDevice->GetRemoteNode("DeviceUserID")->GetString() << std::endl;
        std::cout << std::endl;
    }
    catch (BGAPI2::Exceptions::IException& ex) {
    ...
    try {
        ...
        std::cout << "5.1.10   Announced buffers:       " << bufferList->GetAnnouncedCount()
            << " using " << pBuffer->GetMemSize() * bufferList->GetAnnouncedCount() << " [bytes]" << std::endl;
    }
    catch (BGAPI2::Exceptions::IException& ex) {
        returncode = (returncode == 0) ? 1 : returncode;
        std::cout << "ExceptionType:    " << ex.GetType() << std::endl;
    ...
    try {
        for (int i = 0; i < 10; i++) {
            ...
            // SOFTWARE TRIGGER
            std::cout << " Execute TriggerSoftware " << i + 1 << " is interface type "
                << pDevice->GetRemoteNode("TriggerSoftware")->GetInterface() << std::endl;
