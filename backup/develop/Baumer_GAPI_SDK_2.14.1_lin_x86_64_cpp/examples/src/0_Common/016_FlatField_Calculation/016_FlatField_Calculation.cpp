...
int main(int argc, char* argv[]) {
    ...
    BGAPI2::String sReferenceFilename;
    ...
    while (true) {
        ...
        try {
            for (int i = 0; i < 12; i++) {
                ...
                if (pBufferFilled == NULL) {
                    ...
                    BGAPI2::Ext::Sc::Shading::ReturnCode result =
                        pShading->ReferenceAdd(sBufferPixelFormat, uWidth, uHeight, (char*)(pBufferMem)+uOffset, uBufferSize);
                    ...
                    } else if ((result == BGAPI2::Ext::Sc::Shading::eError_Dark) ||
                        (result == BGAPI2::Ext::Sc::Shading::eError_Bright)) {
        ...
        if (uReferenceCount > 0) {
            ...
            try {
                ...
                std::cout << pReferenceType << " on " << uReferenceCount << " image(s) saved to '"
                    << sReferenceFilename << "'" << std::endl;
            }
            catch (BGAPI2::Exceptions::IException& ex) {
                returncode = (returncode == 0) ? 1 : returncode;
                std::cout << "ExceptionType:    " << ex.GetType() << std::endl;
