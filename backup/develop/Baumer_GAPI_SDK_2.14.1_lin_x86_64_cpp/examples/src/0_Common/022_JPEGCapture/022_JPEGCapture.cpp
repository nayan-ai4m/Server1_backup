...
const std::string kImageCompressionMode = "ImageCompressionMode";
const std::string kImageCompressionModeJPEG = "JPEG";

bool getParameterValue(std::string parameter, unsigned int* iValue);

void printHelp() {
    std::cout << "----" << std::endl;
    std::cout << "Usage:" << std::endl;
    std::cout << "022_JPEGCapture [-n <number>]" << std::endl;
    std::cout << "----" << std::endl;
    std::cout << "-n <number> - specifies the number of images to be captured and saved as jpeg" << std::endl;
    std::cout << std::endl;
}

int main(int argc, char* argv[])
{
    unsigned int num_to_capture = 10;
    bool valid_input = true;
    if (argc > 1) {
        valid_input = false;
        for (int iCount = 0; iCount < argc; iCount++) {
            std::string argument = argv[iCount];
            if (argument.find("-n") != std::string::npos) {
                if (iCount + 1 >= argc || !getParameterValue(argv[iCount + 1], &num_to_capture)) {
                    printHelp();
                    return 1;
                }
                valid_input = true;
    ...
    std::cout << std::endl;
    std::cout << "###############################################################" << std::endl;
    std::cout << "# Example 022_JPEGCapture.cpp                                 #" << std::endl;
    std::cout << "###############################################################" << std::endl;
    std::cout << std::endl << std::endl;
    if (!valid_input) {
        std::cout << "Invalid Parameter Input! Program aborted." << std::endl;
        printHelp();
        return 1;
    }
    ...
    try {
        ...
        if (pDevice->GetRemoteNodeList()->GetNodePresent(kImageCompressionMode.c_str())) {
            if(pDevice->GetRemoteNodeList()->GetNode(kImageCompressionMode.c_str())->IsWriteable()) {
                ...
                std::cout << "         JPEG Supported:          No Feature '" <<
                    kImageCompressionMode << "' not writeable." <<
                    std::endl;
            }
        } else {
            std::cout << "         JPEG Supported:          No Feature '" <<
    ...
    if (jpeg_streaming_capable) {
        ...
        pDevice->GetRemoteNode(kImageCompressionMode.c_str())->SetValue("Off");
    }
...
bool getParameterValue(std::string parameter, unsigned int* iValue) {
    if (parameter.size() > 0) {
        *iValue = strtol(parameter.c_str(), NULL, 10);
    }
    if (*iValue <= 1) {
        return false;
    }
