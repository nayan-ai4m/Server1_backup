...
int main(int argc, char* argv[]) {
    ...
    std::cout << std::endl;
    std::cout << "##############################################################" << std::endl;
    std::cout << "# PROGRAMMER'S GUIDE Example 017_FlatField_Correction.cpp    #" << std::endl;
    std::cout << "##############################################################" << std::endl;
    ...
    static const Argument argumentList[] = {
        { &sPixelFormat, "p", "pixelformat", false, argumentString, 0, "<Pixel Format>",
        "configure device pixel format" },
        ...
        { &sReferenceDark, NULL, "dark", false, argumentString, 0, "<filename>", "path of dark rerefence file" },
    ...
    try {
        ...
        if (bRoiReset) {
            ...
            iRoiX = 0;
            iRoiY = 0;
            iRoiWidth = static_cast<int>(pDevice->GetRemoteNode("SensorWidth")->GetInt());
            iRoiHeight = static_cast<int>(pDevice->GetRemoteNode("SensorHeight")->GetInt());
        }
    ...
    std::cout << std::endl;
    std::cout << "LOADING FLAT FIELD CORRECTION" << std::endl;
    std::cout << "#############################" << std::endl << std::endl;
    ...
    if (sReferenceDark != "") {
        // LOADING DARK FIELD REFERENCE
        try {
            pShading->ReferenceLoad(false, sReferenceDark.c_str());
        }
        catch (BGAPI2::Exceptions::IException& ex) {
            returncode = (returncode == 0) ? 1 : returncode;
            std::cout << "ExceptionType:    " << ex.GetType() << std::endl;
    ...
    // LOADING LIGHT REFERENCE
    try {
        pShading->ReferenceLoad(true, sReferenceLight.c_str());
    }
    catch (BGAPI2::Exceptions::IException& ex) {
        returncode = (returncode == 0) ? 1 : returncode;
        std::cout << "ExceptionType:    " << ex.GetType() << std::endl;
    ...
    // PROCESSING INTERNAL CORRECTION DATA
    try {
        pShading->ReferenceApply();
    }
    catch (BGAPI2::Exceptions::IException& ex) {
        returncode = (returncode == 0) ? 1 : returncode;
        std::cout << "ExceptionType:    " << ex.GetType() << std::endl;
    ...
    // ACTIVATE FLAT FIELD CORRECTION
    try {
        pShading->SetEnable(true);
    }
    catch (BGAPI2::Exceptions::IException& ex) {
        returncode = (returncode == 0) ? 1 : returncode;
        std::cout << "ExceptionType:    " << ex.GetType() << std::endl;
    ...
    try {
        for (int i = 0; i < 12; i++) {
            ...
            if (pBufferFilled == NULL) {
                ...
                if (bImageSaved == false) {
                    try {
                        void* pBufferMem = pBufferFilled->GetMemPtr();
                        ...
                        BGAPI2::Image* pImage = pImageProcessor->CreateImage(
                            (bo_uint)pBufferFilled->GetWidth(),
                            (bo_uint)(pBufferFilled->GetHeight()),
                            pBufferFilled->GetPixelFormat(),
                            reinterpret_cast<char*>(pBufferMem) + uOffset,
                            uBufferSize - uOffset);
                        pImage->GetNode("SaveBrw")->SetString("ImageSample.brw");
