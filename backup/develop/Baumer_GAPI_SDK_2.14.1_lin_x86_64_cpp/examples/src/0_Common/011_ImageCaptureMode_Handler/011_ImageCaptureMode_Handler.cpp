...
int main() {
    ...
    try {
        pDataStream->RegisterNewBufferEventHandler(pDataStream, (BGAPI2::Events::NewBufferEventHandler)&BufferHandler);
    }
    catch (BGAPI2::Exceptions::IException& ex) {
        returncode = (returncode == 0) ? 1 : returncode;
        std::cout << "ExceptionType:    " << ex.GetType() << std::endl;
    ...
    try {
        pDataStream->StartAcquisition(iNumToAcquire);
    ...
    // CAPTURE 12 IMAGES
    std::cout << std::endl;
    std::cout << "CAPTURE 12 IMAGES BY IMAGE CALLBACK" << std::endl;
    std::cout << "###################################" << std::endl << std::endl;

    while (bufferList->GetDeliveredCount() < iNumToAcquire) {
#if defined(_WIN32)
        Sleep(30);
#else
        usleep(30000);
    ...
    try {
        ...
        std::cout << "         ExposureTime:                   " << std::fixed << std::setprecision(0)
            << pDevice->GetRemoteNode(sExposureNodeName)->GetDouble() << " ["
    ...
    try {
        ...
        pDataStream->RegisterNewBufferEvent(BGAPI2::Events::EVENTMODE_UNREGISTERED);
        BGAPI2::Events::EventMode currentEventMode = pDataStream->GetEventMode();
        BGAPI2::String sCurrentEventMode = "";
        switch (currentEventMode) {
        case BGAPI2::Events::EVENTMODE_POLLING:
    ...
    try {
        if (pBufferFilled == NULL) {
            ...
            std::cout << " Image " << std::setw(5) << pBufferFilled->GetFrameID() << " received in memory address "
                << pBufferFilled->GetMemPtr() << std::endl;
            // queue buffer again
            pBufferFilled->QueueBuffer();
        }
    }
