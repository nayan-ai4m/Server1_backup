...
def should_suppress_bad_orientation(machine_name, new_status):
    """
    Suppresses bad_orientation updates if sent within the last 25 seconds for the same machine.
    Internally tracks last sent timestamps per machine.
    """
    if not hasattr(should_suppress_bad_orientation, "_last_sent"):
        should_suppress_bad_orientation._last_sent = {}

    last_sent_time = should_suppress_bad_orientation._last_sent.get(machine_name, datetime.min)

    now = datetime.now()
    suppress = False

    if "bad_orientation" in new_status:
        if now - last_sent_time < timedelta(seconds=25):
            suppress = True
        else:
            should_suppress_bad_orientation._last_sent[machine_name] = now
        ...
        for machine_name in machine_names:
            if machine_name in zone_nodes:
               ...
               # Getting the Current Status of the Node/Zone from Memgraph to check with the New Status
                ...
                # Making sure the bad_orientation_status does not change for 10 secs to avoid multiple messages
                suppress_bad_orientation = should_suppress_bad_orientation(machine_name, new_status)
                
                # Updating with the New Status if the the New Status is different from the Current Status
                if new_status != current_status and not (downgrade_jamming or downgrade_starvation or suppress_bad_orientation):
