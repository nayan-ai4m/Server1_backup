...
def get_initial_stroke_values(db_conn):
    """Get current stroke values from mc30_mid table for fallback/restore"""
    try:
        cursor = db_conn.cursor()
        query = """
        ...
        if result:
            ...
            print(f"✓ Fetched initial stroke values from mc30_mid: S1={s1_value}, S2={s2_value}")
            return s1_value, s2_value
        else:
            print("WARNING: No data found in mc30_mid table")
            return None, None
    except Exception as e:
        print(f"ERROR: Failed to get initial stroke values from mc30_mid: {e}")
        return None, None
...
async def main():
    ...
    try:
        ...
        # Step 3: Get initial stroke values from mc30_mid table
        print("Step 3: Fetching initial stroke values from mc30_mid table...")
        ...
        if initial_s1 is None or initial_s2 is None:
            print("WARNING: Could not fetch initial values from mc30_mid, using sweep start values as fallback")
            initial_s1 = S1_START
            initial_s2 = S2_START
            print(f"Fallback values: S1={initial_s1:.2f}, S2={initial_s2:.2f}")
        else:
            print(f"Initial values (from mc30_mid): S1={initial_s1:.2f}, S2={initial_s2:.2f}")
