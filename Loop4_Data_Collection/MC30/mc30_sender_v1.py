...
class PLCNatsClient:
    ...
    async def write_tag(self, tag_name, value, skip_m82=False, timeout=NATS_TIMEOUT):
        """
        ...
        Args:
            ...
            skip_m82: If True, tells receiver not to trigger M82 (for batch writes)
        ...
        message = {
            ...
            "value": value,
            "status": True,
            "skip_m82": skip_m82  # Add flag to control M82 triggering
        }
    ...
    async def write_s1(self, value, skip_m82=False):
        """Write S1 stroke value"""
        response = await self.write_tag(TAG_S1_NAME, value, skip_m82=skip_m82)
        if response.get("ack"):
            m82_msg = " (M82 skipped)" if skip_m82 else ""
            print(f"✓ Wrote {TAG_S1_NAME} (D{TAG_S1}) = {value:.4f}{m82_msg}")
            return True
        else:
            print(f"ERROR: Failed to write {TAG_S1_NAME}: {response.get('error')}")
            return False

    async def write_s2(self, value, skip_m82=False):
        """Write S2 stroke value"""
        response = await self.write_tag(TAG_S2_NAME, value, skip_m82=skip_m82)
        if response.get("ack"):
            m82_msg = " (M82 skipped)" if skip_m82 else ""
            print(f"✓ Wrote {TAG_S2_NAME} (D{TAG_S2}) = {value:.4f}{m82_msg}")
            return True
        else:
...
async def wait_for_ready_status(plc_client, db_conn, initial_s1, initial_s2):
    ...
    while True:
        ...
        if status == 1:
            ...
            # Write initial values once (batched: S1 without M82, S2 with M82)
            await plc_client.write_s1(initial_s1, skip_m82=True)
            await asyncio.sleep(0.1)
            await plc_client.write_s2(initial_s2, skip_m82=False)
...
async def main():
    ...
    try:
        ...
        for idx, (s1, s2) in enumerate(parameter_pairs):
            ...
            # IMPORTANT: Write S1 first WITHOUT triggering M82, then S2 WITH M82
            # This ensures both values are in place before machine processes them
            write_success = False
            while not write_success:
                print(f"Writing to PLC: S1={s1}, S2={s2} (batched write)")

                # Write S1 first, skip M82 trigger
                success_s1 = await plc_client.write_s1(s1, skip_m82=True)
                ...
                # Small delay to ensure S1 is written before S2
                await asyncio.sleep(0.1)

                # Write S2 second, trigger M82 after this write
                success_s2 = await plc_client.write_s2(s2, skip_m82=False)

                if success_s1 and success_s2:
                    print(f"✓ Successfully wrote S1={s1}, S2={s2}")
                    print(f"✓ M{M82_TRIGGER} triggered after BOTH values written")
                    write_success = True
                else:
            ...
            # Read and record data for PREVIOUS combination (skip first iteration)
            if previous_s1 is not None and previous_s2 is not None:
                cycle_id += 1
                print(f"Reading data for PREVIOUS combination: S1={previous_s1}, S2={previous_s2}")

                avg_load = get_average_instant_load(db_conn)

                if avg_load is not None:
                    append_to_csv(cycle_id, previous_s1, previous_s2, avg_load)
                else:
                    print("WARNING: Could not get average load, writing NULL to CSV")
                    append_to_csv(cycle_id, previous_s1, previous_s2, "NULL")

            # Save current parameters for next iteration
            previous_s1 = s1
            previous_s2 = s2

        # Step 7: Final read for the last combination
        print(f"\n{'='*60}")
        print("Reading data for FINAL combination...")
        print(f"{'='*60}")
        ...
        if avg_load is not None:
            ...
            print("WARNING: Could not get average load for final combination")
            append_to_csv(cycle_id, previous_s1, previous_s2, "NULL")
        ...
        if nc and initial_s1 is not None and initial_s2 is not None:
            try:
                plc_client = PLCNatsClient(nc)
                # Batched write: S1 without M82, then S2 with M82
                await plc_client.write_s1(initial_s1, skip_m82=True)
                await asyncio.sleep(0.1)
                await plc_client.write_s2(initial_s2, skip_m82=False)
                print(f"✓ Restored: S1={initial_s1:.2f}, S2={initial_s2:.2f}")
                print(f"✓ M{M82_TRIGGER} triggered after BOTH values restored")
            except Exception as e:
