...
class PLCNatsClient:
    ...
    async def write_s1(self, value):
        """Write S1 stroke value"""
        response = await self.write_tag(TAG_S1_NAME, value)
        if response.get("ack"):
            print(f"✓ Wrote {TAG_S1_NAME} (D{TAG_S1}) = {value:.4f}")
            return True
        else:
    ...
    async def write_s2(self, value):
        """Write S2 stroke value"""
        response = await self.write_tag(TAG_S2_NAME, value)
        if response.get("ack"):
            print(f"✓ Wrote {TAG_S2_NAME} (D{TAG_S2}) = {value:.4f}")
            return True
        else:
...
def get_average_instant_load(db_conn):
    ...
    try:
        ...
        query = """
            ...
            FROM mc27_fast
            WHERE cam_position BETWEEN 170 AND 200
            ORDER BY timestamp DESC
            LIMIT 30;
        """
        cursor.execute(query)
...
def get_initial_stroke_values(db_conn):
    """Get current stroke values from mc27_mid table for fallback/restore"""
    try:
        cursor = db_conn.cursor()
        query = """
            SELECT hori_sealer_stroke_1, hori_sealer_stroke_2
            FROM mc27_mid
            ORDER BY timestamp DESC
            LIMIT 1;
        """
        ...
        if result:
            s1_value, s2_value = result
            print(f"✓ Fetched initial stroke values from mc27_mid: S1={s1_value}, S2={s2_value}")
            return s1_value, s2_value
        else:
            print("WARNING: No data found in mc27_mid table")
            return None, None
    except Exception as e:
        print(f"ERROR: Failed to get initial stroke values from mc27_mid: {e}")
        return None, None
...
async def check_machine_running(db_conn):
    """Check if machine is running and raise exception if stopped after retries"""
    ...
    if status != 1:
        print(f"⚠️  MACHINE STATUS CHANGED to {status}!")
        print("Attempting to wait for machine to recover...")

        # Retry 6 times with 5 second intervals (30 seconds total)
        max_retries = 6
        retry_interval = 5

        for attempt in range(1, max_retries + 1):
            print(f"Retry {attempt}/{max_retries}: Waiting {retry_interval} seconds...")
            await asyncio.sleep(retry_interval)

            status, _ = get_machine_status(db_conn)

            if status == 1:
                print(f"✓ Machine has recovered! Status is now {status}")
                return
            else:
                print(f"Status still {status}, continuing to wait...")

        # If we get here, machine did not recover after all retries
        print(f"❌ MACHINE DID NOT RECOVER after {max_retries * retry_interval} seconds!")
        raise MachineStoppedException(f"Machine stopped with status={status} and did not recover")
...
async def main():
    ...
    try:
        ...
        # Step 3: Get initial stroke values from mc27_mid table
        print("Step 3: Fetching initial stroke values from mc27_mid table...")
        initial_s1, initial_s2 = get_initial_stroke_values(db_conn)

        # Fallback to sweep start values if database read fails
        if initial_s1 is None or initial_s2 is None:
            print("WARNING: Could not fetch initial values from mc27_mid, using sweep start values as fallback")
            initial_s1 = S1_START
            initial_s2 = S2_START
            print(f"Fallback values: S1={initial_s1:.2f}, S2={initial_s2:.2f}")
        else:
            print(f"Initial values (from mc27_mid): S1={initial_s1:.2f}, S2={initial_s2:.2f}")

        print("NOTE: These values will be used for machine recovery and final restoration")
        ...
        for idx, (s1, s2) in enumerate(parameter_pairs):
            ...
            while not write_success:
                ...
                if success_s1 and success_s2:
                    ...
                    print(f"✓ M{M82_TRIGGER} automatically triggered by receiver")
                    write_success = True
                else:
            ...
            # Additional buffer time before reading data
            print("Waiting 10 seconds buffer time before reading data...")
        ...
        if nc and initial_s1 is not None and initial_s2 is not None:
            ...
            # Retry configuration for restore
            max_retries = 5 if machine_stopped else 1
            retry_wait = 30 if machine_stopped else 0
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    success_s1 = await plc_client.write_s1(initial_s1)
                    await asyncio.sleep(0.1)
                    success_s2 = await plc_client.write_s2(initial_s2)
                    ...
                    if success_s1 and success_s2:
                        ...
                        raise Exception("Write returned failure")

                except Exception as e:
                    print(f"ERROR: Failed to restore fallback values (attempt {attempt + 1}/{max_retries}): {e}")
